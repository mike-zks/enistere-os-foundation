...
it('computes an honest Platform Baseline v2 gap for all seven existing runtimes', () => {
  ...
  assert.equal(report.summary.nextjs.missing, 0);
  assert.equal(report.summary.angular.missing, 0);
  assert.equal(report.summary.nextjs.partial, 0);
  assert.equal(report.summary.angular.partial, 0);
