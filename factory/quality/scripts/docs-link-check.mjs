...
function runCli() {
  ...
  if (result.brokenLinks.length === 0) {
    console.log(`Documentation link check passed (${result.checkedFiles.length} files).`);
    return 0;
  }

  console.error(`Documentation link check failed (${result.brokenLinks.length} broken links).`);
