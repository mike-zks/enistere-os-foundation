...
export function validatePromptDefinition(prompt, index = 0) {
  ...
  if (!isSafeRelativePath(prompt.path) || !prompt.path.startsWith('factory/ai/prompts/')) {
    addIssue(issues, `${base}.path`, 'path must be a safe relative path under factory/ai/prompts/');
  }
