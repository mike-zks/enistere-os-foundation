/**
 * Tests for Factory AI governed prompt execution runner.
 *
 * Run:
 *   node --test factory/ai/runtime/test/prompt-runner.test.mjs
 */
...
async function createFixture() {
  ...
  await mkdir(join(repoRoot, 'factory', 'ai', 'prompts', 'global'), { recursive: true });
  await writeFile(join(repoRoot, 'docs', 'mission.md'), 'Mission context without secrets.\n', 'utf8');
  await writeFile(join(repoRoot, 'factory', 'ai', 'prompts', 'global', 'execution.md'), 'Prompt body template.\n', 'utf8');
  ...
  const prompt = {
    ...
    role: 'executor',
    scope: 'global',
    path: 'factory/ai/prompts/global/execution.md',
    allowedInputs: ['mission-brief'],
    requiredDocuments: ['docs/mission.md'],
    allowedFiles: ['factory/ai/prompts/global/execution.md'],
...
test('runs an active prompt through the fake provider and returns a valid report', async () => {
  ...
  assert.deepEqual(
    ...
    ['docs/mission.md', 'factory/ai/prompts/global/execution.md'],
  );
