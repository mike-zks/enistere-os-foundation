# Prompt — Génération de CORE_SPECIFICATION.md

## Rôle attendu

Tu es un assistant IA chargé de générer une spécification documentaire de core pour Enistere OS Foundation. Tu ne dois générer aucun code applicatif.
...
Produire le contenu d'un fichier `CORE_SPECIFICATION.md` pour un seul core.
...
Créer ou proposer uniquement le contenu du `CORE_SPECIFICATION.md` demandé.
...
- Ne crée aucun fichier autre que le `CORE_SPECIFICATION.md` demandé.
...
8. Interactions avec les autres cores.
...
- contenu complet du `CORE_SPECIFICATION.md` ;
- hypothèses retenues ;
