...
## 4. Redirection anonyme

Destination : **`/?auth=required`** (constante `PROTECTED_ANONYMOUS_REDIRECT`). Chemin **interne
statique** : aucune page `/login` (Web Auth 5), aucun `returnUrl` libre, aucune open redirect, aucun token.
...
| # | Cas | Vérifié |
|---|---|---|
| a | anonyme `GET /protected` | **redirection serveur** (`NEXT_REDIRECT` + meta refresh → `?auth=required`), **aucune** donnée privée |
