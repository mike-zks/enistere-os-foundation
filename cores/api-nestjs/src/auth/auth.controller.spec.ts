...
function buildAuthServiceMock() {
  return {
    ...
    getAuthorizationSummary: jest
      .fn()
      .mockResolvedValue({ roles: ['administrator'], permissions: ['users.read'] }),
  };
}
...
describe('AuthController (me)', () => {
  ...
  it('GET /auth/me/authorization returns role and permission codes', async () => {
    await request(app.getHttpServer())
      .get('/auth/me/authorization')
      .expect(200)
      .expect(({ body }) => {
        expect(body.data).toEqual({ roles: ['administrator'], permissions: ['users.read'] });
      });

    expect(authService.getAuthorizationSummary).toHaveBeenCalledWith('user-1');
  });
});
