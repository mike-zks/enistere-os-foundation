...
describe('AuthService', () => {
  ...
  let authorizationContext: { load: jest.Mock };
  ...
  beforeEach(() => {
    ...
    const configService = { get: () => 900 } as unknown as ConfigService<AppConfig, true>;
  ...
  describe('getAuthorizationSummary', () => {
    it('returns role and permission codes for the UI', async () => {
      await expect(service.getAuthorizationSummary('user-1')).resolves.toEqual({
        roles: ['administrator'],
