...
## 2quater. Publication + intégration cores (ADR-016) — **P1 (hors `cores/api-nestjs/`)**
...
- **Dépendances** : ADR-013 (CI/CD), ADR-014 (registry/images). **Hors `cores/api-nestjs/`**
...
adaptateurs de session). Ces travaux relèvent du dépôt/des cores clients, **hors `cores/api-nestjs/`**.
