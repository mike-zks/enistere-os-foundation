...
**packages clients officiels** (`packages/`) validés localement, non publiés. **CI** (3 workflows) : `ci.yml` (niveau 1 — non-régression monorepo) + `api-runtime-ci.yml` (niveau 2 — **runtime API NestJS** : PostgreSQL + MinIO jetables, migrations, unit + e2e, OpenAPI check) + `web-e2e-ci.yml` (niveau 3 — **E2E navigateur** : stack réelle + **Playwright/Chromium** ; Health/Auth/Files) **+ `registry-ci.yml`** (niveau 4 partiel — **registry GHCR** : Dockerfiles API/Web multi-stage non-root, build + push images sur `main`, tags immuables, **sans déploiement**) ; **gouvernance** (Cloud Core 4). **Cloud Core** = `IMPLEMENTATION_PARTIELLE` (CC1 cadrage + CC2 runtime API + CC3 E2E + CC4 gouvernance + CC5 registry ; **sans déploiement/secret applicatif**). ADR-013 partiel (niveaux 1–4 partiel) ; **ADR-014 partiel** (registry GHCR) ; protection de branche `main` documentée (à appliquer manuellement, repo public). État détaillé et vérifié : [`docs/project-status/`](docs/project-status/README.md).
