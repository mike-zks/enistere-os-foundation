...
**packages clients officiels** (`packages/`) validés localement, non publiés. **CI** : `.github/workflows/ci.yml` (niveau 1 — non-régression monorepo) **+ `api-runtime-ci.yml`** (niveau 2 — **CI runtime API NestJS** : PostgreSQL + MinIO jetables, migrations, unit + e2e, OpenAPI check) ; **Cloud Core** = `IMPLEMENTATION_PARTIELLE` (Cloud Core 1 cadrage + Cloud Core 2 CI runtime ; **sans déploiement/Docker/registry/secret**). ADR-013 partiel (niveaux 1–2) ; ADR-014 non implémenté. État détaillé et vérifié : [`docs/project-status/`](docs/project-status/README.md).
