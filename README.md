...
**packages clients officiels** (`packages/`) validés localement, non publiés. **CI** (3 workflows) : `ci.yml` (niveau 1 — non-régression monorepo) + `api-runtime-ci.yml` (niveau 2 — **runtime API NestJS** : PostgreSQL + MinIO jetables, migrations, unit + e2e, OpenAPI check) **+ `web-e2e-ci.yml`** (niveau 3 — **E2E navigateur** : stack réelle API + PostgreSQL + MinIO + Web + **Playwright/Chromium** ; Health/Auth/Files) ; **gouvernance** (Cloud Core 4 : **7 checks** `main` documentés + politiques artefacts/couverture/pinning). **Cloud Core** = `IMPLEMENTATION_PARTIELLE` (CC1 cadrage + CC2 runtime API + CC3 E2E + CC4 gouvernance ; **sans déploiement/Docker/registry/secret**). ADR-013 partiel (niveaux 1–3) ; ADR-014 non implémenté ; **protection de branche `main` documentée, à appliquer manuellement**. État détaillé et vérifié : [`docs/project-status/`](docs/project-status/README.md).
