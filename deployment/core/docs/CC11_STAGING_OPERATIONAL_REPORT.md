...
`deployment/core/staging/scripts/backup-postgres.sh` — lit `POSTGRES_USER` / `POSTGRES_DB` depuis
...
`deployment/core/staging/scripts/backup-minio.sh` — utilise `minio/mc mirror` via conteneur
...
`deployment/core/staging/scripts/rotate-smoke-account.sh` — génère `crypto.randomBytes(32).toString('base64url')`, hache en `argon2id` avec les paramètres du `.env.staging`, met à jour
...
|---------|------|-------------|
| `deployment/core/staging/scripts/backup-postgres.sh` | Script | Backup pg_dump → .sql.gz horodaté |
| `deployment/core/staging/scripts/backup-minio.sh` | Script | Backup minio/mc mirror |
| `deployment/core/staging/scripts/rotate-smoke-account.sh` | Script | Rotation argon2id, valeur non conservée |
| `deployment/core/docs/CC11_OPERATIONAL_RUNBOOK.md` | Runbook | Procédures opérationnelles complètes |
| `deployment/core/docs/CC11_STAGING_OPERATIONAL_REPORT.md` | Rapport | Ce document — preuves d'exécution |
