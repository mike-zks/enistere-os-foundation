# Deployment — E2E navigateur Web (niveau 3)

> **IMPLÉMENTÉ (Deployment 3)** — workflow **`.github/workflows/web-e2e-ci.yml`** + suite Playwright
> (`starters/nextjs/e2e/`). Ce document a servi de plan ; il décrit désormais le niveau 3 réel. Toujours
> **sans** déploiement, registry/GHCR, secret GitHub. Aligné sur `CLOUD_CORE_V1_EXECUTION_BASELINE.md` §13, la
> revue `starters/nextjs/docs/WEB_CORE_V1_INCREMENT_REVIEW.md` (réserve « E2E navigateur ») et ADR-013.

## 0. Implémentation réelle (`web-e2e-ci.yml` + `starters/nextjs/e2e/`)
