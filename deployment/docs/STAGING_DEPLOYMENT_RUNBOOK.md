# Runbook — Déploiement staging manuel (Deployment 6)
...
> Réf. : `deployment/staging/`, `GHCR_REGISTRY_GUIDE.md`, `SECRETS_POLICY.md`.
...
> ✅ **Stratégie tranchée (Deployment 8) — migrations DEPUIS L'IMAGE (Option A)** : l'image runtime **embarque**
...
# git fetch && git checkout <sha-déployé> && cd starters/nestjs && npm ci
...
> ✅ **Défaut image API CORRIGÉ (Deployment 8)** : le query engine Prisma était compilé pour **OpenSSL 1.1.x**
...
#   éventuellement via starters/nestjs/scripts/proof-seed-user.ts + un upload de test, données éphémères.
```
