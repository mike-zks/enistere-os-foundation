# Runbook opérationnel staging (Deployment 11)
...
```bash
bash deployment/staging/scripts/backup-postgres.sh \
...
```bash
bash deployment/staging/scripts/backup-minio.sh \
  /home/deploy/enistere-staging/.env.staging \
...
```bash
bash deployment/staging/scripts/rotate-smoke-account.sh \
  /home/deploy/enistere-staging/.env.staging \
  <smoke-account-email>
```
