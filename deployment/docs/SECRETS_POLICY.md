# Deployment — Politique des secrets (cadrage)
...
> valeur). Aligné sur `DEPLOYMENT_SPECIFICATION.md` §28, ADR-005/007/040 et `strategy/07_SECURITY.md`.
...
- **Staging manuel (Deployment 6)** : les secrets sont fournis **sur le serveur staging** via un fichier
  `.env.staging` **non versionné** (copié depuis `deployment/staging/.env.staging.example`, placeholders
