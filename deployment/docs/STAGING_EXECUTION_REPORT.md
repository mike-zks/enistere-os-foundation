# Rapport — Exécution staging contrôlée (Deployment 9)
...
- **Files (métadonnées + URL signée via l'API)** : **non exercé** (dépend d'un utilisateur authentifié + d'un
  ...
  **joignable par le navigateur** — est **confirmé** (l'hôte atteint MinIO à `<staging-host>:9000`). En revanche
  ...
  exercée. → **à valider sur serveur (Deployment 10)**.

## 6. MinIO / URL signée (§10) — décision appliquée

**Option A** (Deployment 7) appliquée : `S3_ENDPOINT = http://<staging-host>:9000` (adresse de l'hôte, **joignable
...
**IP brute** reste à confirmer avec le **presign de l'API** (et/ou un **domaine** MinIO) — Deployment 10.
...
configuration sur Internet sans HTTPS + pare-feu + domaine (Deployment 10).
...
(ni « réelle sur serveur », ni production-ready). **Prochaine action : Deployment 10 — préparation serveur
