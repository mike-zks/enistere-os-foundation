# Rapport — Dry-run staging contrôlé (Deployment 7)
...
| Compose | `deployment/staging/docker-compose.staging.example.yml` (inchangé) |
...
**Deployment 8 — corriger l'image runtime API NestJS (moteur de requête Prisma) pour qu'elle démarre**, puis
...
# Deployment 8 — correction du moteur Prisma & re-validation (2026-06-11)

## 8.1 Correction appliquée

- **`starters/nestjs/prisma/schema.prisma`** — generator : `binaryTargets = ["native", "debian-openssl-3.0.x"]`.
...
- **`starters/nestjs/Dockerfile`** — installation d'**`openssl`/`ca-certificates` AUSSI au stage build**
...
- L'**image GHCR corrigée n'est pas encore reconstruite/publiée** (rebuild local impossible — egress npm). Elle
  sera **rebâtie et poussée par la registry CI** (gate `api-smoke`) **au merge** de Deployment 8. La preuve
...
## 8.5 Confirmation post-merge (Deployment 8B, 2026-06-11)
