# Runbook — Rollback staging manuel (Deployment 6)
...
> ✅ **Pré-requis (Deployment 8)** : un rollback ne vaut que vers une image **qui démarre**. Le défaut de boot
...
# 1. Backup préventif
bash <repo>/deployment/staging/scripts/backup-postgres.sh \
...
- `deployment/staging/scripts/backup-postgres.sh` — backup pg_dump gzip horodaté
- Restore test : voir `CC11_OPERATIONAL_RUNBOOK.md` §3
- Backup MinIO : `deployment/staging/scripts/backup-minio.sh`
