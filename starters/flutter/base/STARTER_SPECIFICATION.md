# starter Flutter — Spécification du Core

## 1. Résumé exécutif

Le **starter Flutter** définit le socle mobile Flutter de référence pour les futures applications Enistere.
...
Le starter Flutter cadre la base commune des applications mobiles Flutter Enistere.
...
- préparer l'intégration UI Kit (tokens Enistere via ADR-034), API Core, Deployment et Factory Quality ;
- fournir un périmètre extensible pour maps, tracking, offline, notifications et modules avancés ;
- rester cohérent avec le starter React Native en termes de intentions (tokens, états UI, sécurité, accessibilité) sans dupliquer son implémentation.

## 3. Objectifs du starter Flutter
...
- Standardiser Dio pour les appels HTTP vers l'starter NestJS.
...
- Compatible avec les contrats OpenAPI de l'starter NestJS.
...
- Compatibilité starter NestJS `/auth/login` / `/auth/refresh` / `/auth/logout`.
- Aucun token dans les logs.
...
- Compatibilité starter NestJS (headers Bearer, refresh token HTTP response).
...
|---|---|---|---|
| Flutter 1 | Core specification (CETTE MISSION) | `STARTER_SPECIFICATION.md` + `README.md` | ADR-034 validé |
...
## 31. Cohérence avec starter React Native

Le starter Flutter partage les **intentions** du starter React Native, pas l'implémentation :
