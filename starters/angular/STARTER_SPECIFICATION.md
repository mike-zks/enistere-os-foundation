# starter Angular — Spécification du Core

## 1. Résumé exécutif

Le **starter Angular** définit le socle web Angular de référence pour les futures interfaces Enistere de type backoffice, dashboard administratif, SI interne, portail opérateur et outil métier complexe.
...
Le rôle du starter Angular est de cadrer la base commune des applications web Angular Enistere.
...
- préparer l'intégration avec starter NestJS, starter Spring Boot, Deployment, UI Kit, Factory Quality et Documentation ;
...
## 3. Objectifs du starter Angular
...
- Compatibilité starter NestJS `/api/v1/auth/login` / `/api/v1/auth/refresh` / `/api/v1/auth/logout`.
...
- Aligné avec starter NestJS (ADR-006) — helpers OR/AND sans wildcard.
...
- Headers de sécurité configurés côté Deployment (CSP, X-Frame-Options, HSTS).
...
- Compatibilité starter NestJS et starter Spring Boot (même contrat `/auth`).
...
Le starter Angular doit s'intégrer avec starter NestJS et starter Spring Boot :
...
## 26. Intégration Deployment

Le starter Angular doit s'intégrer avec Deployment pour :
...
Le starter Angular consomme les tokens Enistere depuis `packages/ui-kit/tokens/` :
...
Le starter Angular partage les **intentions** des autres cores, pas l'implémentation :
...
|---|---|---|---|
| Angular 1 | Core specification (CETTE MISSION) | `STARTER_SPECIFICATION.md` + `README.md` | ADR-035 validé |
...
- CSP strict configurable côté Deployment (Angular compile en bundle statique — pas d'`eval`).
