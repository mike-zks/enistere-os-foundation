...
export class AuthService {
  ...
  constructor(
    ...
    this.accessTtlSeconds = configService.get('jwtAccessTtl', { infer: true });
  }
  ...
  async login(
    ...
    try {
      ...
      await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_LOGIN_FAILED, metadata, {});
      throw error;
    }
    ...
    try {
      ...
      throw new InternalServerErrorException({
        code: ERROR_CODES.AUTH_SESSION_CREATION_FAILED,
    ...
    await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_LOGIN_SUCCEEDED, metadata, {
      actorId: user.id,
      subjectId: user.id,
  ...
  /**
   * Résumé d'autorisation (codes de rôles + permissions effectives) pour l'UI
   * conditionnelle. L'API reste l'autorité finale ; ce résumé n'est pas une preuve.
   */
  getAuthorizationSummary(userId: string): Promise<AuthorizationSummary> {
    return this.authorizationContext.load(userId).then((context) => ({
  ...
  async refresh(refreshToken: string, metadata: RefreshSessionMetadata = {}): Promise<RefreshResult> {
    ...
    if (!parsed) {
      await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_REFRESH_FAILED, metadata, {
    ...
    if (session.revokedAt) {
      if (session.revocationReason === SessionRevocationReason.ROTATED) {
        ...
        await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_REFRESH_REUSE_DETECTED, metadata, {
          subjectId: session.userId,
          data: { familyId: session.familyId },
        });
        await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_SESSION_FAMILY_REVOKED, metadata, {
          subjectId: session.userId,
    ...
    await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_REFRESH_SUCCEEDED, metadata, {
      actorId: user.id,
      subjectId: user.id,
  ...
  async logout(refreshToken: string, metadata: RefreshSessionMetadata = {}): Promise<void> {
    ...
    if (parsed) {
      ...
      if (session && this.refreshTokenService.matches(parsed.secret, session.tokenHash)) {
        ...
        await this.recordAudit(AUDIT_EVENT_TYPES.AUTH_LOGOUT, metadata, {
