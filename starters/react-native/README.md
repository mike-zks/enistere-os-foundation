...
| **Smoke runtime local reproductible (RN 29)** | `scripts/smoke-android.js` + `npm run smoke:android` | Script Node stdlib uniquement : préflight `adb`/`npx`/device, mock auth local temporaire sur `127.0.0.1:3000` (handlers `POST /auth/login`, `POST /auth/refresh`, **`POST /files`**), `adb reverse`, lancement Expo Android, interaction semi-automatisée par labels UI Android (`uiautomator`) et rapport JSON structuré. Rejoue public sign-in → Home protégé → Settings → scroll diagnostics → retour Home → **Upload diagnostics (RN36 : fixture `enistere-smoke.txt` via `adb shell`, `Submit diagnostic upload`, `Upload complete`)** → refresh session → sign out. **Pas un E2E complet** : dépend d'un émulateur/device + Expo Go et ne remplace pas une future décision Detox/Maestro/Appium/Playwright mobile. Aucun backend réel, endpoint métier, SDK/adaptateur natif réel, dépendance, persistance, retry ou changement Auth/Query. |
...
| **Upload sécurisé (RN 7)** | `src/upload` | `MobileFile` `{uri,name,type}` + helpers purs (`isMobileFile`, `describeFileForLog` → **`{type,extension}`**, sans `uri` ni nom brut, RN 8, `isAllowedFileType` UX) ; **`useUploadMutation`** (via `useAuthedMutation` → `apiClient.files.upload`, pont 401 `authedRequest`, `FormData` reconstruit au retry). **Mutation** → pas de clé de cache ; **aucun fichier/URL signée/token en cache/log/store** ; validation **backend autoritaire** (ADR-007). **Aucun endpoint métier.** |
| **Upload diagnostics screen (RN 36)** | `app/(app)/upload.tsx` | Écran protégé générique prouvant le parcours upload runtime (gap B1 MOBILE_CORE_V1_READINESS_REVIEW.md) : formulaire **RHF + Zod** pour la sélection de catégorie, fichier fixture smoke `enistere-smoke.txt` (descripteur `MobileFile` sans picker), appel **`useUploadMutation`** via le client officiel, états **`LoadingState`/`MessageState`/`ErrorState`**. **Aucune URL signée/token/payload serveur/URI device en log, cache ou store** (ADR-007/015) ; catégorie uniquement affichée en succès (valeur choisie par l'utilisateur, non retournée par le serveur). Lien depuis Home. |
...
| Tests | `test/` | **`node --test`** sur le cœur agnostique (… clipboard, retry, **telemetry-context-gate, telemetry-service**) — **367 cas `test(...)`** |
...
│   │   ├── home.tsx          # écran placeholder authentifié + liens Settings + Upload
│   │   ├── settings.tsx      # settings/diagnostics génériques protégés (RN 26)
│   │   └── upload.tsx        # upload diagnostics screen RN36 (gap B1)
