...
**Raison :** c'est la **convention réelle du repo**. `starters/nextjs` et
`starters/nestjs` placent leur `package.json`, `src/` et leur configuration à
...
aux `workspaces` racine. C'est cohérent avec `starters/nestjs` (autonome) et
...
(`starters/react-native/**` + docs de statut), et ADR-016 §7 reporte la
