# starter Spring Boot — Spécification du Core

## 1. Résumé exécutif

Le **starter Spring Boot** définit le socle backend Java de référence pour les futures APIs Enistere orientées enterprise, finance, administration et systèmes d'information.
...
Le starter Spring Boot cadre la base backend Java des projets Enistere enterprise.
...
- Rester cohérent avec l'starter NestJS en termes de contrats, de sécurité et de standards — sans dupliquer son implémentation.
...
- la confusion avec l'starter NestJS — les deux cores sont complémentaires, pas concurrents.

---

## 5. Périmètre fonctionnel

Le starter Spring Boot couvre le socle technique commun suivant :
...
- intégration avec Deployment, Mobile Core, Web Core et Factory Quality.
...
- web : cookie `HttpOnly`, `Secure` en production, `SameSite=Strict` ou `Lax` selon stratégie (cf. starter Next.js BFF Auth) ;
...
- structurés (JSON via Logback + logstash-logback-encoder si observabilité Deployment activée) ;
...
- stratégie de scaling (sticky sessions ou Redis pub/sub) à valider avec Deployment.
...
La base cible est **PostgreSQL** (cf. Deployment).
...
- readiness/liveness distincts si Deployment cible Kubernetes ;
...
L'observabilité avancée (traces distribuées, dashboards Grafana) est coordonnée avec Deployment — hors périmètre V1.
...
## 35. Intégration avec Deployment

Le starter Spring Boot doit s'intégrer avec Deployment pour :
...
- variables d'environnement injectées par Deployment (secrets manager ou env vars Docker/Compose) ;
...
## 37. Intégration avec starter Next.js
...
## 38. Intégration avec Factory Quality et Documentation

- **Factory Quality** : appliquer les standards de tests JUnit/Testcontainers, lint, couverture critique, validation CI/CD. Le scope `quality-gates` doit couvrir le build Java et les tests.
- **Documentation** : maintenir `STARTER_SPECIFICATION.md`, `README.md`, guides, ADR et runbooks liés au core dans le format Documentation.
...
- `STARTER_SPECIFICATION.md` — cette spécification ;
...
| Observabilité | Micrometer + Prometheus vs autre | Deployment V2+ |
| CI Java | GitHub Actions `mvn verify` ou `gradle build` | Mission Spring Boot 3+ |
| Distribution | Docker image Java / Deployment | Mission Spring Boot V1 final |
...
|---|---|---|---|
| Spring Boot 1 | Core specification (CETTE MISSION) | `STARTER_SPECIFICATION.md` + `README.md` | ADR applicables lus |
...
## 42. Cohérence avec starter NestJS
