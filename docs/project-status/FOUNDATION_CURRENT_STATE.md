...
| Mobile Core Flutter | **`mobile-flutter`** — **`IMPLEMENTATION_AVANCEE`** (Flutter V1 Readiness Review, 2026-07-14 ; Flutter 7 B1 fermé, 2026-07-14) : §29 5/11 satisfaits (navigation, upload, thème, tests, API Dio). **B1 FERMÉ** — `android/` + smoke `emulator-5554` 5/5 ✅. Bloquants restants : B2 `flutter_secure_storage` absent, B3 `RefreshInterceptor` absent, B4 UI states absents, B5 login form absent. Réserves acceptées : R1 iOS Linux, R2 pas de backend réel, R3 Freezed délibérément absent. Rapports : `MOBILE_FLUTTER_V1_READINESS_REVIEW.md`, `MOBILE_FLUTTER7_ANDROID_SMOKE_REPORT.md`. Prochain : Flutter 8 — SecureStorage seam + adapter. |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-flutter/      IMPLEMENTATION_AVANCEE (Flutter V1 Readiness Review — §29 5/11 ; Flutter 7 B1 fermé — android/ + smoke emulator-5554 5/5 ✅ ; B2→B5 bloquants restants, Flutter 8→11 vers VALIDE_V1)
