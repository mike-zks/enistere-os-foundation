...
| Core mobile (socle) | **`mobile-react-native`** — **API_CLIENT_INTEGRATED** : socle **Expo SDK 55** / Expo Router (RN 1) + auth/session durci (RN 2) + forms/validation/offline (RN 3) **+ RN 4 — client officiel** : **`@enistere/api-client-fetch` + `@enistere/api-contracts` réellement consommés** (liés `file:` + `metro.config.js`, **core autonome — root non touché**) ; `MobileAuthSessionAdapter` (**injection Bearer** de l'access token en mémoire, le client ne stocke aucun token) + `EnistereAuthApi` (POST `/auth/login`+`/auth/refresh` via `client.raw` typé) ; **AuthEngine préservé** (refresh coalescé + expiration proactive) ; **`enableRefresh:false`** (une seule stratégie) + **pont 401 `withAuthRetry`/`authedRequest` (RN 4B)** : `401`→`refreshSession` coalescé→1 retry→purge ; erreurs `ApiClientError` ; **47 tests `node --test`** (incl. `with-auth-retry`) ; **typecheck + lint + test 47/47 + expo-doctor 19/19 + `expo export` ios** verts ; **aucune logique métier** |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  API_CLIENT_INTEGRATED (Expo SDK 55 + Expo Router : app/ + src/{api,auth,config,forms,navigation,offline,query,states,storage,theme,types,ui} + metro.config.js + test/ node --test ; AuthEngine + SessionStore + 401-refresh ; forms RHF+Zod ; offline queue mémoire ; client OFFICIEL @enistere/api-client-fetch lié file: ; autonome hors workspaces)
...
puis **RN 4 — intégration réelle du client officiel RÉALISÉ** (`mobile-react-native` → **API_CLIENT_INTEGRATED** ;
`@enistere/api-client-fetch` + `@enistere/api-contracts` **consommés** — liés `file:` + Metro, **sans** ajout aux
workspaces racine ; `MobileAuthSessionAdapter` (injection Bearer) + `EnistereAuthApi` réel ; **AuthEngine préservé**
(refresh coalescé + expiration + 401→retry, `enableRefresh:false`) ; **47 tests `node --test`**, typecheck/lint/
test/doctor verts + **bundle `expo export` ios** prouvant l'intégration). Cloud Core reste **PAUSE_CONTROLEE**,
staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 5 — server-state data
layer (hooks TanStack Query au-dessus du client)** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (API_CLIENT_INTEGRATED : auth/session + forms/validation + offline préparatoire + **client officiel intégré**) ≠ implémentation complète (V1 partielle : Zustand, upload, notifications, hooks server-state, offline sync réelle — différés).
