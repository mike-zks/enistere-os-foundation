...
| Quality Core | **`quality-core`** — **IMPLEMENTATION_PARTIELLE** (Quality Core V2 Readiness Review + QC7, 2026-07-12) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md`, `BRANCH_PROTECTION_RUNBOOK.md`, **`RELEASE_PROCESS_RUNBOOK.md`**, **`AI_PROMPT_GOVERNANCE.md`**, templates GitHub PR/Issues, 3 checklists, `scripts/quality-gates.mjs` + tests (**36/36**) ; prompts catalogués ; processus utilisé pour publier `foundation-v1.0.0`. **Protection `main` active via GitHub Rulesets** (`protect-main`, enforcement `active`, 8 checks requis). Aucun workflow modifié, aucune dépendance, aucun changement runtime. |
| Docs Core | **`docs-core`** — **IMPLEMENTATION_PARTIELLE** (Docs Core 1→4 + V2 Readiness Review, 2026-07-12) : `CORE_SPECIFICATION.md`, `README.md`, index central `docs/README.md` (strategy, ADR, project-status, runbooks, prompts, quality gates) + audit `DOCS_CORE_NAVIGATION_AUDIT.md` + onboarding `CONTRIBUTOR_ONBOARDING.md` + glossaire `GLOSSARY.md` + script `check-doc-links.mjs` testé + revue `DOCS_CORE_V2_READINESS_REVIEW.md`. Decision : maintien `IMPLEMENTATION_PARTIELLE` tant que les guides principaux et l'onboarding complet ne sont pas livres. Aucun runtime applicatif, workflow, dépendance, RAG, génération ou site documentaire. |
...
| Foundation baseline | **`FOUNDATION_V1_RELEASED`** — release `foundation-v1.0.0` publiée le 2026-07-12 ; revue `FOUNDATION_V1_BASELINE_READINESS_REVIEW.md` ; API/Web/UI Kit `VALIDE_V1`, packages API `IMPLEMENTATION_AVANCEE`, Quality Core `IMPLEMENTATION_PARTIELLE`, CI L1-L4 verte sur `main`, ruleset `protect-main` actif |
...
enistere-os-foundation/
  ...
  cores/
    ...
    quality-core/      IMPLEMENTATION_PARTIELLE (gates, checklists, runbooks, templates, prompts, release process utilisé)
    docs-core/         IMPLEMENTATION_PARTIELLE (index central docs + onboarding + glossaire + link check + V2 readiness review)
...
| `docs-core` | oui | oui | **oui** (script link check) | **IMPLEMENTATION_PARTIELLE** |
| `mobile-flutter` | oui (vide) | non | non | **DOSSIER_SEULEMENT** |
| `quality-core` | oui | oui | **oui** (gouvernance qualité : gates, checklists, runbooks, templates, release process utilisé ; 36 tests script) | **IMPLEMENTATION_PARTIELLE** |
