...
primitives génériques**, puis **RN 15 — app lifecycle primitives génériques RÉALISÉ** (`mobile-react-native` →
**APP_LIFECYCLE_READY** ; modèle **`AppLifecycleState`** (`active`/`background`/`inactive`/`unknown`) +
`normalizeAppLifecycleState` (RN `AppStateStatus` incl. `extension`→`background` ; **tolère tout input invalide** →
`unknown`) + helpers purs `isForeground`/`isBackground`/**`isValidTransition`** (matrice : même état no-op ;
`unknown`→n'importe ; **un état déterminé ne revient jamais à `unknown`** ; états réels interchangeables)/
`nextAppLifecycleState` ; `AppLifecycleAdapter` (seam RN `AppState`) + **`AppLifecycleAdapterError`** contrôlé +
**placeholder** mémoire + `createAppLifecycleService` (`getState`/`subscribe`/`transition`/`dispose`, transitions
**validées** ; **best-effort non-intrusif** — erreurs adapter `getState`/`subscribe` **capturées** + `warn` sûr, défaut
`unknown`, **listener qui throw isolé** ; **logs RN 8 sûrs** : que des **enums** `{from,to}`/`{operation}` — **aucune
donnée utilisateur**) ; **aucun `AppState` réel/provider global obligatoire/stockage/dépendance** ; **212 tests
`node --test`**, typecheck/lint/test/doctor + `git diff --check` verts ; RN 15 **n'ajoute aucune dépendance**). Cloud
Core reste **PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React
Native 16 — connectivité réseau (network status) primitives génériques** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (APP_LIFECYCLE_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales + i18n/localisation + deep-linking/routing + analytics/télémétrie + accessibilité a11y + app lifecycle**) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs natifs réels (permissions/notifications/localisation/linking/AccessibilityInfo/AppState), catalogues métier i18n + routes concrètes, SDK analytics réel, application des props a11y dans des composants, câblage des effets lifecycle, backend d'observabilité, offline sync réelle — différés).
