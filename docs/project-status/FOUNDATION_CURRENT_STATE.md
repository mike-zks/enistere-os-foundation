...
| Packages officiels | `@enistere/api-contracts` (**12 tests**), `@enistere/api-client-fetch` (**30 tests**) (validés **localement**, non publiés ; **instanciés (public + authentifié/BFF)** dans le Web Core — preuve API réelle ; **`FilesApi.list()` ajouté (Files 5)**, `files_list` dans schema.ts ; **distribution décidée** le 2026-07-17 : GitHub Packages npm registry, repli GitHub Release tarballs — aucune publication effectuée) |
...
GitHub Packages npm registry pour `@enistere/*`. Repli : GitHub Release tarballs. Les packages restent
`private:true` et non publiés jusqu'à une mission de préparation publish-ready puis publication contrôlée.
