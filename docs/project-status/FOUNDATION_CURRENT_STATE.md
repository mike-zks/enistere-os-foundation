...
techniques réelles** : API Core, Web Core, Cloud Core, UI Kit, Docs Core et Quality Core sont
**VALIDE_V1** ; le Mobile Core React Native est **IMPLEMENTATION_AVANCEE**. Les cores encore vides
restent `ai-core`, `api-spring`, `mobile-flutter` et `web-angular`.
...
| Core mobile (socle) | **`mobile-react-native`** — **IMPLEMENTATION_AVANCEE** (Mobile Core V1 Readiness Review, 2026-07-13) : socle Expo SDK 55 / Expo Router RN 1→36. Primitives RN 1→25, Settings RN26, shell RN27, smoke Android RN28/RN29/RN34B, préflight iOS RN30 bloqué Linux, RN31 en attente macOS/Xcode, RN32 formulaire sign-in RHF/Zod, RN33 thème, RN34 patch Expo SDK, RN35 alignement UI Kit, **RN36 upload diagnostics screen (gap B1 fermé : `app/(app)/upload.tsx`, `useUploadMutation` via client officiel, RHF+Zod catégorie, `LoadingState`/`MessageState`/`ErrorState`, smoke `POST /files` mock + fixture `enistere-smoke.txt` + `Upload complete` vérifié)**. Preuves : typecheck + lint + **test 367/367** `node --test` + expo-doctor **19/19** + `expo export -p ios` + `git diff --check` verts. `npm run smoke:android` **passed** (étend RN29 avec upload). `npm run smoke:ios` blocked (Linux). `VALIDE_V1` différé : parité iOS non exécutée, store natif non sensible encore en seam MMKV/AsyncStorage. Critères §9.4 **8/8 satisfaits** (B1 fermé par RN36). |
...
| Cores vides | `ai-core`, `api-spring`, `mobile-flutter`, `web-angular` |
...
> **Mise à jour Mobile Core V1 Readiness Review** (2026-07-13) : le core
> `mobile-react-native` passe de **`STARTER_UI_KIT_ALIGNED`** à
> **`IMPLEMENTATION_AVANCEE`**. Rapport :
> `docs/project-status/MOBILE_CORE_V1_READINESS_REVIEW.md`. La V1 complète reste différée
> par l'upload runtime mobile non prouvé, le smoke iOS bloqué par l'environnement Linux et
> le store natif non sensible encore en seam.
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  IMPLEMENTATION_AVANCEE (Expo SDK 55 + Expo Router ; primitives RN 1→25 ; Settings RN26 ; shell RN27 ; smoke Android RN28/RN29/RN34B ; préflight iOS RN30 bloqué Linux ; RN31 en attente macOS ; RN32 sign-in RHF+Zod ; RN33 thème ; RN34 patch Expo SDK ; RN35 tokens alignés UI Kit + LoadingView/EmptyView/ErrorView + 13 tests ; RN36 upload diagnostics screen + smoke POST /files ; Mobile V1 review 2026-07-13 ; §9.4 8/8 ; typecheck/lint/test 367/367/expo-doctor 19/19/export iOS verts)
