...
succès iOS artificiel. `web-angular` est **`HTTP_SERVER_STATE_READY`** (Angular 4,
2026-07-16 — couche HTTP générique : `AppApiError` typée, `ErrorInterceptor`, `LogInterceptor` (jamais body/Authorization/query params sensibles/URL signée), `RequestState<T>` + `createRequestState()` RxJS, `APP_BASE_URL` token ; ordre intercepteurs `auth → log → error` ; build SUCCESS + tests complets). Le seul core encore vide est `ai-core`.
...
| Cores vides | `ai-core` ; `web-angular` passe à **`HTTP_SERVER_STATE_READY`** (Angular 4, 2026-07-16 — `AppApiError` + `mapHttpError` + `isAppApiError` ; `ErrorInterceptor` (HttpErrorResponse → AppApiError, 401 surfacé) ; `LogInterceptor` (method+path+status+durée, jamais body/Authorization/query params sensibles/URL signée) ; `RequestState<T>` factories + `createRequestState()` ; `APP_BASE_URL` token ; ordre `auth → log → error` ; tests 76+nouvelles specs ✅ ; prochaine action : Web Core Angular 5 — Reactive Forms + Angular Material) |
| CI/CD, conteneurisation | **CI niveaux 1–3 + registry (niveau 4 partiel) + L5 api-spring (99/99 ✅ TC) + CC10 staging HTTPS réel VALIDÉ** : `ci.yml` + `api-runtime-ci.yml` + `web-e2e-ci.yml` + **`registry-ci.yml`** (images GHCR publiques) + **`api-spring-ci.yml`** (L5 : Java 21 Temurin + Maven Wrapper + 71 tests Testcontainers) ; **Dockerfiles** API/Web ; **CC10** : `docker-compose.cc10.yml`, reverse proxy compatible Traefik + Let's Encrypt HTTP-01, `sha-5bf4c0f`, 4 conteneurs `healthy`, `staging.enistere.com` + `s3-staging.enistere.com` HTTPS, auth BFF + upload + URL signée + téléchargement **bout-en-bout validés** |
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               → vide
    web-angular/           HTTP_SERVER_STATE_READY (Angular 4, 2026-07-16 — HTTP + AppApiError + ErrorInterceptor + LogInterceptor + RequestState<T> RxJS)
  packages/
...
| `web-angular` | oui | **oui** (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15) | **oui** (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state — AppApiError, ErrorInterceptor, LogInterceptor, RequestState<T>, APP_BASE_URL) | **HTTP_SERVER_STATE_READY** |
