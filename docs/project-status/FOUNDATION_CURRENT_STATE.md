...
processus projet derive + matrice de profils stack API/Web/Mobile/Cloud. Les profils API + mobile
(`nestjs-react-native`, `spring-flutter`, `spring-react-native`, `nestjs-flutter`) sont explicitement
reconnus ; aucun generateur CLI, runtime ou template de bootstrap n'est encore livré.
...
| Project Factory | **`SPECIFICATION_DOCUMENTAIRE`** — processus projet derive + matrice profils stack (`nestjs-next`, `spring-angular`, profils API+mobile, profils hybrides) ; aucun generateur/template/runtime |
