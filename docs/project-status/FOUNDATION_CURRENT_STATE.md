...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`IMPLEMENTATION_AVANCEE`** (AI Core 9, 2026-07-16 — §18 7/7 satisfaits ; §19 futur avancé ; registre de prompts local + validateur Node pur + redaction layer pure + context builder allow-list + provider seam/fake provider déterministe + evaluation harness local + décision Retrieval/RAG V1 + schema de rapport d'execution + runner de prompt gouverné fake-provider ; B1 fermé ; `VALIDE_V1` différé par B2 citation retrieval + B3 runbook ; aucun provider réel, SDK IA, LLM judge, embedding, vector DB, RAG runtime, endpoint, secret, dépendance ou réseau).
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               IMPLEMENTATION_AVANCEE (AI Core 9 — B1 prompt runner fermé ; VALIDE_V1 différé par B2/B3)
...
| `ai-core` | oui | oui (`CORE_SPECIFICATION.md`, `README.md`) | **oui** (`prompt-registry.json`, validateur Node pur, redaction layer, context builder, provider fake, evaluation harness, décision Retrieval/RAG V1, schema rapport d'execution, runner prompt fake-provider) | **IMPLEMENTATION_AVANCEE** |
