...
succès iOS artificiel. Les cores vides restent `ai-core` et `web-angular`. `api-spring` est **`IMPLEMENTATION_PARTIELLE`**, sous-statut `CI_JAVA_READY` (Spring Boot 5, 2026-07-15 : PostgreSQL + Spring Data JPA + Flyway V1+V2 + Argon2id + RBAC + OpenAPI springdoc 2.8.6 + upload MinIO/S3 compatible + StoredFile + DTO public sans fuite + FakeStorageService test ; `./mvnw verify` 71/71 ✅ ; gate CI L5 `api-spring-ci.yml` + scope `api-spring` quality-gates 42/42).
...
enistere-os-foundation/
  ...
  cores/
    ...
    api-spring/           IMPLEMENTATION_PARTIELLE / CI_JAVA_READY (Spring Boot 5, 2026-07-15 — PostgreSQL + JPA + Flyway V1+V2 + Argon2id + RBAC + OpenAPI + upload MinIO/S3 + FakeStorageService ; 71/71 ✅ ; L5 api-spring-ci.yml + scope quality-gates 42/42)
...
| `api-spring` | oui | oui | **oui** (Spring Boot 4/5 — JPA + Flyway V1+V2 + Argon2id + RBAC + OpenAPI springdoc + upload `POST /api/v1/files/upload` + FakeStorageService ; 71/71 ✅ ; gate CI L5 + scope quality-gates) | **IMPLEMENTATION_PARTIELLE** (sous-statut `CI_JAVA_READY`, 2026-07-15) |
