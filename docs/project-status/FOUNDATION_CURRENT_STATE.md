...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`IMPLEMENTATION_AVANCEE`** (AI Core 11, 2026-07-17 — §18 7/7 satisfaits ; §19 futur avancé ; registre de prompts local + validateur Node pur + redaction layer pure + context builder allow-list + provider seam/fake provider déterministe + evaluation harness local + décision Retrieval/RAG V1 + schema de rapport d'execution + runner de prompt gouverné fake-provider + citations retrieval + runbook d'usage ; B1/B2/B3 fermés ; promotion `VALIDE_V1` à décider par revue finale ; aucun provider réel, SDK IA, LLM judge, embedding, vector DB, RAG runtime, endpoint, secret, dépendance ou réseau).
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               IMPLEMENTATION_AVANCEE (AI Core 11 — B1/B2/B3 fermés ; V1 final decision recommandée)
