...
**RN 4/4B — client officiel intégré + pont 401**, puis **RN 5 — couche server-state RÉALISÉ**
(`mobile-react-native` → **SERVER_STATE_READY** ; client officiel `@enistere/api-client-fetch` consommé (`file:` +
Metro, **root non touché**) + adaptateur de session + `EnistereAuthApi` ; **AuthEngine préservé** ; **pont 401**
`authedRequest` ; **couche server-state TanStack Query générique** : `createQueryKeys`, `useAuthedQuery`/
`useAuthedMutation` via `authedRequest`, `toQueryError` (sans donnée sensible), `purgeServerState` au logout —
**401 jamais retenté, mutations sans retry, pas de persistance** ; **59 tests `node --test`**, typecheck/lint/
test/doctor verts + **bundle `expo export` ios**). Cloud Core reste **PAUSE_CONTROLEE**, staging
**EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 6 — état local (Zustand) +
câblage `signOut → purgeServerState`** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (SERVER_STATE_READY : auth/session + forms/validation + offline préparatoire + **client officiel intégré + couche server-state**) ≠ implémentation complète (V1 partielle : Zustand, upload, notifications, logger, offline sync réelle — différés).
