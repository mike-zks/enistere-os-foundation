...
| Core mobile (socle) | **`mobile-react-native`** — **IMPLEMENTATION_AVANCEE** (Mobile Core V1 Readiness Review, 2026-07-13) : socle Expo SDK 55 / Expo Router RN 1→35. Primitives RN 1→25, Settings RN26, shell RN27, smoke Android RN28/RN29/RN34B, préflight iOS RN30 bloqué Linux, RN31 en attente macOS/Xcode, RN32 formulaire sign-in RHF/Zod, RN33 thème, RN34 patch Expo SDK, RN35 alignement UI Kit. Preuves : typecheck + lint + **test 367/367** `node --test` + expo-doctor **19/19** + `expo export -p ios` + `npm audit` 0 vuln + `git diff --check` verts. `npm run smoke:android` **passed** (`emulator-5554` / Pixel_6a, 2026-07-08). `npm run smoke:ios` blocked (Linux). `VALIDE_V1` différé : upload runtime mobile non prouvé, parité iOS non exécutée, store natif non sensible encore en seam MMKV/AsyncStorage. |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  IMPLEMENTATION_AVANCEE (Expo SDK 55 + Expo Router ; primitives RN 1→25 ; Settings RN26 ; shell RN27 ; smoke Android RN28/RN29/RN34B ; préflight iOS RN30 bloqué Linux ; RN31 en attente macOS ; RN32 sign-in RHF+Zod ; RN33 thème ; RN34 patch Expo SDK ; RN35 tokens alignés UI Kit + LoadingView/EmptyView/ErrorView + 13 tests ; Mobile V1 review 2026-07-13 ; typecheck/lint/test 367/367/expo-doctor 19/19/export iOS/audit verts)
