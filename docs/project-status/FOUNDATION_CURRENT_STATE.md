...
status) primitives génériques**, **RN 17 — feature flags / config primitives génériques**, puis **RN 18 — gate
biométrique local primitives génériques RÉALISÉ** (`mobile-react-native` → **BIOMETRIC_GATE_READY** ; **ajoute
`src/biometrics`** — **gate d'UX local** (ADR-015 §20) : **ne remplace JAMAIS** login/refresh/session serveur
(**l'API Core reste l'autorité**), reste **optionnel** + **fallback projet**, **rien de sensible stocké ou loggé** ;
ajouts : `BiometricAvailability` (`available`/`notEnrolled`/`unsupported`/`unknown`) + `BiometricType` **borné**
(`fingerprint`/`facial`/`iris`/`unknown` — jamais un descripteur natif identifiant) + `BiometricAuthOutcome`
(`success`/`refused`/`cancelled`/`lockout`/`unavailable`/`error`) ; helpers **tolérants** (`normalizeBiometric*` :
alias/booléens → enum ; **junk → `unknown`/`error`, jamais `success`**) + `normalizeAvailabilityState`/
`normalizeAuthResult` → objets **gelés** + `isAvailabilityUsable`/`isAuthSuccess` ; `BiometricAdapter` (seam Expo
`LocalAuthentication`/Keychain, **async** ; `reason` du prompt **forwardé, jamais loggé**) + **`BiometricAdapterError`**
contrôlé (`operation` seul) + **placeholder** mémoire (`setAvailability`/`setNextOutcome` + compteur
`authenticateCalls`) + `createBiometricService` (`getAvailability`/`isAvailable`/`authenticate` ; **stateless** ;
**aucun faux succès** — `authenticate` vérifie la disponibilité **d'abord** et renvoie `unavailable` **sans prompter**
si inutilisable, adapter qui throw → `error`, outcome inconnu → `error`, **ne throw jamais** ; **logs RN 8 sûrs**
`{availability,type}`/`{outcome}`/`{operation}` — **jamais le prompt ni la cause native**) ; **aucun
`LocalAuthentication`/Keychain réel/secret/biométrie/résultat/profil stocké** ; **262 tests `node --test`**,
typecheck/lint/test/doctor + `git diff --check` verts ; RN 18 **n'ajoute aucune dépendance**). Cloud Core
reste **PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 19 —
crash / error-reporting primitives génériques (seam, sans SDK réel — ADR-019)** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (BIOMETRIC_GATE_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales + i18n/localisation + deep-linking/routing + analytics/télémétrie + accessibilité a11y + app lifecycle + connectivité réseau + feature flags/config + gate biométrique local**) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs natifs réels (permissions/notifications/localisation/linking/AccessibilityInfo/AppState/NetInfo/LocalAuthentication/Keychain), catalogues métier i18n + routes concrètes, SDK analytics réel, application des props a11y dans des composants, câblage des effets lifecycle, offline sync réelle, source remote-config/local réelle des feature flags, activation biométrique réelle + fallback concret (par projet, documenté — ADR-015 §20/§31), backend d'observabilité — différés).
