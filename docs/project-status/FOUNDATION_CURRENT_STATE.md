...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`IMPLEMENTATION_PARTIELLE`** (AI Core 8, 2026-07-16 — registre de prompts local + validateur Node pur + redaction layer pure + context builder allow-list + provider seam/fake provider déterministe + evaluation harness local + décision Retrieval/RAG V1 + schema de rapport d'execution ; aucun provider réel, SDK IA, LLM judge, embedding, vector DB, RAG runtime, endpoint, secret, dépendance ou réseau).
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               IMPLEMENTATION_PARTIELLE (AI Core 8 — prompt registry + redaction + context + provider fake + evaluation harness + décision Retrieval/RAG V1 + schema rapport, aucun provider réel)
...
| `ai-core` | oui | oui (`CORE_SPECIFICATION.md`, `README.md`) | **oui** (`prompt-registry.json`, validateur Node pur, redaction layer, context builder, provider fake, evaluation harness, décision Retrieval/RAG V1, schema rapport d'execution) | **IMPLEMENTATION_PARTIELLE** |
