...
| Cores documentaires | **`quality-core`** — **SPECIFICATION_DOCUMENTAIRE** (Quality Core 2, 2026-07-11) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md` (matrice 8 cores × 11 gates). Checklists : `docs/checklists/PR_QUALITY_CHECKLIST.md`, `RELEASE_READINESS_CHECKLIST.md`, `CORE_STATUS_REVIEW_CHECKLIST.md`. **Quality Core 2** : `scripts/quality-gates.mjs` (Node 24, sans dépendance — `list` / `plan <scope>` / `run <scope>` ; 7 scopes : `docs`/`packages`/`ui-kit`/`web`/`root-audit`/`mobile-static`/`all-safe`) + `scripts/quality-gates.test.mjs` (**36/36 tests node:test**). Aucun workflow modifié, aucune dépendance, aucun changement runtime. _(aucun autre core documentaire ; `mobile-react-native` est passé au starter ci-dessus)_ |
