...
succès iOS artificiel. Les cores vides restent `ai-core` et `web-angular`. `api-spring` est **`VALIDE_V1`** (Spring Boot 7 — SB7 AuditModule + URL signée + CORS env var, 2026-07-15 : §30 14/15 ✅ / 1/15 ⚠️ (C10 Redis différé) / 0 ✗ ; B1 audit logs ✅ fermé (SB7) ; B2 URL signée ✅ fermée (SB7) ; C15 CORS env var ✅ fermée (SB7, wildcard CORS ignoré avec credentials) ; `./mvnw verify` 90/90 ✅ ; CI L5 active).

| Catégorie | État |
|---|---|
| Stratégie (Phase 0) | 10 documents présents |
| ADR | 20 ADR rédigés et **Validés** (001–016, 034, 039, 040, **041**) ; ADR-017→033 et ADR-035→038 = backlog non rédigé |
...
enistere-os-foundation/
  ...
  docs/
    adr/               20 ADR (001–016, 034, 039, 040, 041) + ADR_BACKLOG + ADR_V1_BLOCKING_REVIEW
  ...
  cores/
    ...
    api-spring/           VALIDE_V1 (SB7 AuditModule + URL signée + CORS env var 2026-07-15 — 14/15 §30 ✅ ; 90/90 ✅ ; L5 CI)
...
| `api-spring` | oui | oui | **oui** (Spring Boot 1→7 — JPA + Flyway V1+V2+V3 + Argon2id + RBAC + OpenAPI + upload + URL signée + AuditModule + CORS env var + FakeStorageService ; 90/90 ✅ ; CI L5) | **VALIDE_V1** (SB7, 2026-07-15 — 14/15 §30 ✅ ; B1+B2+C15 fermés ; R5 Redis différé) |
...
**20 ADR rédigés et Validés** : ADR-001..016, ADR-034, ADR-039, ADR-040, **ADR-041** (build system Maven, 2026-07-14). ADR-017→033 et ADR-035→038 sont **listés dans
