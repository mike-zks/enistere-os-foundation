...
Mobile Core React Native sont **VALIDE_V1**. Le **Mobile Core Flutter** est **`UPLOAD_READY`** (Flutter 5, 2026-07-14). Les cores vides restent `ai-core`, `api-spring` et `web-angular`.
...
| Mobile Core Flutter | **`mobile-flutter`** — **`UPLOAD_READY`** (Mobile Core Flutter 5, 2026-07-14) : `AppFile` descriptor pur + `SafeFileDescriptor` + `describeFileForLog` (jamais path/nom brut) + `isAllowedUploadContentType` + `FileCategory` (9 valeurs) + `UploadedFileMetadata` DTO + `DioUploadService` (`FormData` + `MultipartFile` injectable, boundary Dio automatique, jamais Content-Type forcé). Erreurs : 413→`TooLargeError`, 415→`UnsupportedTypeError`. `http_parser: ^4.0.0`. Tests 120/120. |
...
| V3 | **En cours** — ADR-034 validé ; Flutter 1 spec ✅ ; Flutter 2 starter ✅ ; Flutter 3 auth shell ✅ ; Flutter 4 Dio client ✅ ; Flutter 5 upload multipart ✅ (`UPLOAD_READY`) ; prochaine : Flutter 6 tests + smoke |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-flutter/      UPLOAD_READY (Flutter 5 — AppFile + FileCategory + UploadService + 120 tests)
...
| `mobile-flutter` | oui | oui | oui (starter + auth shell + Dio client + upload primitives) | **UPLOAD_READY** (Flutter 5, 2026-07-14 — `AppFile` + `FileCategory` + `UploadedFileMetadata` + `DioUploadService` + 120 tests) |
