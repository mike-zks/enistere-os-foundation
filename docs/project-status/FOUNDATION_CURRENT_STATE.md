...
`git diff --check` verts ; RN 23 **n'ajoute aucune dépendance**). Cloud Core reste **PAUSE_CONTROLEE**, staging
**EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 24 — retry / backoff primitives
génériques (purs, horloge injectée)** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
