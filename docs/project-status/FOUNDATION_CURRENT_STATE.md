...
Mobile Core React Native sont **VALIDE_V1**. Le **Mobile Core Flutter** est **`TEST_WIDGET_PASSED`** (Flutter 6, 2026-07-14). Les cores vides restent `ai-core`, `api-spring` et `web-angular`.
...
| Mobile Core Flutter | **`mobile-flutter`** — **`TEST_WIDGET_PASSED`** (Mobile Core Flutter 6, 2026-07-14) : 136/136 tests headless (flutter test) — widget tests SplashScreen/SignInScreen/HomeScreen/RouterGuards/AppStartup + unit tests API/Auth/Upload. `integration_test/smoke_test.dart` livré (5 scénarios) + `scripts/smoke.sh` ; intégration device bloquée (library sans dossiers platform — réserve architecturale documentée). iOS bloqué (hôte Linux — réserve environnementale). Rapport : `MOBILE_FLUTTER6_SMOKE_REPORT.md`. |
...
| V3 | **En cours** — ADR-034 validé ; Flutter 1 spec ✅ ; Flutter 2 starter ✅ ; Flutter 3 auth shell ✅ ; Flutter 4 Dio client ✅ ; Flutter 5 upload multipart ✅ ; Flutter 6 tests + smoke ✅ (`TEST_WIDGET_PASSED`) ; prochaine : Flutter V1 Readiness Review |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-flutter/      TEST_WIDGET_PASSED (Flutter 6 — widget tests + smoke report + 136 tests)
...
| `mobile-flutter` | oui | oui | oui (starter + auth shell + Dio client + upload primitives + widget/integration tests) | **TEST_WIDGET_PASSED** (Flutter 6, 2026-07-14 — 136 tests headless + integration_test + smoke.sh + rapport) |
