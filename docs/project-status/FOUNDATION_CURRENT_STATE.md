...
succès iOS artificiel. Les cores vides restent `ai-core` et `web-angular`. `api-spring` est **`IMPLEMENTATION_PARTIELLE`**, sous-statut `FILE_UPLOAD_READY` (Spring Boot 4, 2026-07-15 : PostgreSQL + Spring Data JPA + Flyway V1+V2 + Argon2id + RBAC + OpenAPI springdoc 2.8.6 + upload MinIO/S3 compatible + StoredFile + DTO public sans fuite + FakeStorageService test ; `./mvnw verify` 71/71 ✅).
