...
| Quality Core | **`quality-core`** — **IMPLEMENTATION_AVANCEE** (Quality Core Advanced Readiness Review + release helper, 2026-07-12) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md`, `BRANCH_PROTECTION_RUNBOOK.md`, **`RELEASE_PROCESS_RUNBOOK.md`**, **`AI_PROMPT_GOVERNANCE.md`**, templates GitHub PR/Issues, 3 checklists, `scripts/quality-gates.mjs` + tests, `scripts/release-helper.mjs` + tests ; prompts catalogués ; processus utilisé pour publier `foundation-v1.0.0` ; gates documentaires consommes par Docs Core. **Protection `main` active via GitHub Rulesets** (`protect-main`, enforcement `active`, 8 checks requis). Aucun workflow modifié, aucune dépendance, aucun changement runtime. |
...
| Foundation baseline | **`FOUNDATION_V1_RELEASED`** — release `foundation-v1.0.0` publiée le 2026-07-12 ; revue `FOUNDATION_V1_BASELINE_READINESS_REVIEW.md` ; API/Web/UI Kit `VALIDE_V1`, packages API `IMPLEMENTATION_AVANCEE`, Quality Core `IMPLEMENTATION_AVANCEE`, CI L1-L4 verte sur `main`, ruleset `protect-main` actif |
...
enistere-os-foundation/
  ...
  cores/
    ...
    quality-core/      IMPLEMENTATION_AVANCEE (gates, checklists, runbooks, templates, prompts, release process utilisé, helper release)
...
| `quality-core` | oui | oui | **oui** (gouvernance qualité : gates, checklists, runbooks, templates, prompts, release process utilisé ; scripts `quality-gates` + `release-helper` testés) | **IMPLEMENTATION_AVANCEE** |
