...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`IMPLEMENTATION_AVANCEE`** (AI Core V1 Readiness Review, 2026-07-16 — §18 7/7 satisfaits ; §19 futur 5/8 satisfaits + 3/8 partiels ; registre de prompts local + validateur Node pur + redaction layer pure + context builder allow-list + provider seam/fake provider déterministe + evaluation harness local + décision Retrieval/RAG V1 + schema de rapport d'execution ; `VALIDE_V1` différé ; aucun provider réel, SDK IA, LLM judge, embedding, vector DB, RAG runtime, endpoint, secret, dépendance ou réseau).
...
| `ai-core` | oui | oui (`CORE_SPECIFICATION.md`, `README.md`) | **oui** (`prompt-registry.json`, validateur Node pur, redaction layer, context builder, provider fake, evaluation harness, décision Retrieval/RAG V1, schema rapport d'execution) | **IMPLEMENTATION_AVANCEE** |
