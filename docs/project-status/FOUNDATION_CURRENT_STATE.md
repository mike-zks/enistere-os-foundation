...
| Core mobile (socle) | **`mobile-react-native`** — **STARTER_RUNTIME_HARDENED** : socle Expo SDK 55 / Expo Router RN 1→27. Primitives RN 1→25 inchangées, Settings protégé RN26, shell public/protégé/settings durci RN27. Preuves : typecheck + lint + `npm test` 54 fichiers `node --test` + expo-doctor 19/19 + `expo export -p ios` + `git diff --check` verts. Aucun réseau, endpoint métier, SDK/adaptateur natif réel, dépendance, retry branché ni changement Auth/Query. |
...
> **Mise à jour RN27** : le statut courant du core `mobile-react-native` est
> **`STARTER_RUNTIME_HARDENED`**. RN27 durcit le shell Expo
> public/protégé/settings sans nouvelle primitive ni logique métier : boutons
> bornés/full-width, labels réductibles, conteneurs Sign-in/Home contraints et
> lignes Settings wrap-safe. Aucun réseau, endpoint métier, SDK réel, adaptateur
> natif réel, dépendance, persistance nouvelle ni retry branché. Vérification
> locale : typecheck, lint, test, doctor 19/19, `expo export -p ios` et
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  STARTER_RUNTIME_HARDENED (Expo SDK 55 + Expo Router ; primitives RN 1→25 inchangées ; Settings protégé RN26 ; shell public/protégé/settings durci RN27 : boutons bornés, conteneurs Sign-in/Home contraints, lignes Settings wrap-safe ; aucune logique métier, aucun réseau/dépendance/SDK/adaptateur natif réel/retry branché ; typecheck/lint/test/doctor/export iOS verts)
...
| `mobile-react-native` | oui | oui | **oui** (Expo SDK 55 + Expo Router ; socle RN 1→25 ; Settings protégé RN26 ; durcissement runtime RN27 du shell public/protégé/settings : boutons bornés, conteneurs Sign-in/Home contraints, lignes Settings wrap-safe ; aucun réseau/dépendance/SDK/adaptateur natif réel/endpoint métier/retry branché) | **STARTER_RUNTIME_HARDENED** |
...
volontairement non ajouté). **Prochaine action** : **Mobile Core React Native 28 — smoke visuel device/simulateur du
starter** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (**STARTER_RUNTIME_HARDENED** : primitives RN 1→25 + Settings protégé RN26 + shell public/protégé/settings durci RN27) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs natifs réels, catalogues métier i18n + routes concrètes, SDK analytics/crash réels, application exhaustive des props a11y, offline sync réelle, remote-config réel, biométrie réelle, store préférences natif, UI consentement, backend d'observabilité — différés).
