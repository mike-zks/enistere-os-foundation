...
> **Etat V2 actif — 2026-07-18** : ADR-042 remplace la taxonomie historique des cores par une
> Project Factory AI-native. La migration est en cours sur `refactor/foundation-v2-ai-native` : six
> starters, kernel `enistere`, agents locaux sous double approbation, capability packs, deployment
> packs et matrice golden 18 profils. Les tableaux V1 ci-dessous restent des preuves historiques.
...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`VALIDE_V1`** (AI Core V1 Final Readiness Decision, 2026-07-17 — §18 7/7 satisfaits ; §19 8/8 satisfaits ; registre de prompts local + validateur Node pur + redaction layer pure + context builder allow-list + provider seam/fake provider déterministe + evaluation harness local + décision Retrieval/RAG V1 + schema de rapport d'execution + runner de prompt gouverné fake-provider + citations retrieval + runbook d'usage ; B1/B2/B3 fermés ; aucun provider réel, SDK IA, LLM judge, embedding, vector DB, RAG runtime, endpoint, secret, dépendance, trace storage ou réseau).
...
`project-factory` est désormais cadré en **SPECIFICATION_DOCUMENTAIRE** (2026-07-18) :
processus projet derive + matrice de profils stack API/Web/Mobile/Cloud + templates documentaires
(`FUNCTIONAL_BRIEF`, `TECHNICAL_BLUEPRINT`, `STACK_DECISION`, `SECURITY_NOTES`, `RELEASE_PLAN`).
Les profils API + mobile (`nestjs-react-native`, `spring-flutter`, `spring-react-native`, `nestjs-flutter`)
sont explicitement reconnus ; aucun generateur CLI, runtime applicatif ou exemple derive bout-en-bout n'est
encore livré.

| Catégorie | État |
|---|---|
| Stratégie (Phase 0) | 10 documents présents |
| ADR | **22 ADR rédigés et Validés** (001–016, **034**, **035**, 039, 040, **041**, **042**) ; ADR-017→033 et ADR-036→038 = backlog non rédigé |
| Core implémenté | **API Core NestJS** — **VALIDE_V1** (API Core V1 Readiness Review, 2026-07-12 ; **386 tests unitaires** + e2e CI runtime) |
| Core avancé | **UI Kit** (`@enistere/ui-kit`, v0.1.1) — **VALIDE_V1** (UI Kit VALIDE_V1 review, 2026-07-11) : tokens **+ 19 primitives Web React** accessibles (Button/Input/Label/Text/Spinner/VisuallyHidden + Alert/Card/FormField + Dialog/Select/Toast, UI Kit 4 + Badge/Divider/Skeleton, UI Kit 5 + **LoadingState/EmptyState/ErrorState/SuccessState**, UI Kit 6) ; **181 tests**, a11y ; aligné **React 19** ; **consommé par le Web Core VALIDE_V1** ; cohérence mobile/web prouvée par RN35 (tokens verbatim + 13 tests d'alignement) ; §12.4 4/4 + §59 9/9 |
...
| Packages officiels | `@enistere/api-contracts` (**12 tests**), `@enistere/api-client-fetch` (**30 tests**) (**distribués par GitHub Release tarballs**, tag `packages-api-typescript-v0.1.0` ; GitHub Packages npm registry reste cible future ; **instanciés (public + authentifié/BFF)** dans le Web Core — preuve API réelle ; **`FilesApi.list()` ajouté (Files 5)**, `files_list` dans schema.ts ; premier exemple consommateur executable `examples/api-client-node` livré ; quickstart développeur 15 minutes livré) |
| Cloud Core | **`deployment/core`** — **VALIDE_V1** (Cloud Core 12, 2026-07-12 ; Redis reporté post-V1/V2, `docker-compose.cc10.yml` compose serveur/staging V1 officiel) : `api-runtime-ci.yml` (PostgreSQL+MinIO jetables, migrations, unit+e2e, openapi:check) **+ `web-e2e-ci.yml`** (stack réelle API+PG+MinIO+Web + **Playwright/Chromium** : Health/Auth/Files) + **`registry-ci.yml`** (images GHCR immuables + `api-smoke`) + **`docker-compose.cc10.yml` (reverse proxy compatible Traefik + Let's Encrypt ; `sha-5bf4c0f` ; 4 conteneurs `healthy` ; `staging.enistere.com` + `s3-staging.enistere.com` accessibles HTTPS ; auth BFF 200, upload MinIO VALIDATED, URL signée + téléchargement 200 — **bout-en-bout validé**)** + **CC11 (socle opérationnel vérifié) : health HTTPS 200 ×3 + TLS Let's Encrypt OK ; backup PostgreSQL 4.7 Ko gzip + restore validé (comptages lignes) ; backup MinIO + restore test objet PASSED ; rollback `sha-484f98d` healthy + roll-forward `sha-5bf4c0f` healthy ; rotation compte smoke argon2id (valeur non conservée) ; scripts + runbook + rapport versionnés**. Production/monitoring/Redis standardisé restent V2+. |
...
| Cores vides | aucun |
| CI/CD, conteneurisation | **CI niveaux 1–3 + registry (niveau 4 partiel) + L5 api-spring (99/99 ✅ TC) + L6 web-angular + CC10 staging HTTPS réel VALIDÉ** : `ci.yml` + `api-runtime-ci.yml` + `web-e2e-ci.yml` + **`registry-ci.yml`** (images GHCR publiques) + **`api-spring-ci.yml`** (L5 : Java 21 Temurin + Maven Wrapper + 71 tests Testcontainers) + **`web-angular-ci.yml`** (L6 : Karma/ChromeHeadless + build + audit) ; **Dockerfiles** API/Web ; **CC10** : `docker-compose.cc10.yml`, reverse proxy compatible Traefik + Let's Encrypt HTTP-01, `sha-5bf4c0f`, 4 conteneurs `healthy`, `staging.enistere.com` + `s3-staging.enistere.com` HTTPS, auth BFF + upload + URL signée + téléchargement **bout-en-bout validés** |
...
| Project Factory | **`SPECIFICATION_DOCUMENTAIRE`** — processus projet derive + matrice profils stack (`nestjs-next`, `spring-angular`, profils API+mobile, profils hybrides) + templates documentaires ; aucun generateur/runtime applicatif |
...
| V3 | **Mobile Core Flutter VALIDE_V1** (2026-07-14) — Flutter 1→11 + V1 Final Readiness Decision. §29 9/11 + 2 PARTIAL iOS R1 ; B1→B5 tous fermés ; 218/218 tests headless · smoke `emulator-5554` 7/7 ✅ ; R1 iOS Linux réserve environnementale acceptée. Rapport : `MOBILE_FLUTTER_V1_FINAL_READINESS_DECISION.md`. **API Core Spring Boot 1 SPECIFICATION_DOCUMENTAIRE** (2026-07-14) — `CORE_SPECIFICATION.md` 42 sections + `README.md` ; aucun starter Java. **ADR-041 — Build system Spring Boot : Maven** (2026-07-14) — Maven retenu comme build system V1 (`pom.xml`, Spring Boot Parent POM, `mvn verify`, Maven Wrapper) ; `docs/adr/ADR-041-build-system-api-spring-maven-vs-gradle.md`. **API Core Spring Boot 2 STARTER_INITIALISE** (2026-07-15) — Spring Boot 4.1.0 + Java 21 + Maven ; Spring Security 7.x STATELESS + JWT filter (JJWT 0.12.6) ; auth shell stub (credentials config, vrai JWT, sans DB) ; `./mvnw verify` 18/18 ✅ ; aucun secret hardcodé. **API Core Spring Boot 3 PERSISTENCE_RBAC_READY** (2026-07-15) — PostgreSQL + Spring Data JPA + Flyway V1 + Argon2id (ADR-039) ; entités User/Role/Permission/RefreshToken ; refresh tokens persistés SHA-256 ; RBAC `@PreAuthorize` + permissions JWT stateless ; Testcontainers singleton ; `./mvnw verify` 43/43 ✅. **API Core Spring Boot 4 FILE_UPLOAD_READY** (2026-07-15) — OpenAPI springdoc 2.8.6 (`/v3/api-docs`, Swagger UI, Bearer JWT) ; `FilesConfig` env vars ; migration Flyway `V2__add_stored_files.sql` ; entité `StoredFile` + enums + `StoredFileRepository` ; DTO public sans fuite (`storageKey`/`bucket`/`signedUrl`/`ownerId` internes) ; `StorageService` interface + `MinioStorageService` + `FakeStorageService` test ; `FileService` (MIME whitelist 14 types, taille, `storageKey` 100% serveur) ; `POST /api/v1/files/upload` multipart sécurisé ; 413/415 handlers ; `./mvnw verify` **71/71 ✅**. Non livré (SB5) : liste/delete/download, MinIO TC, Tika, CI Java. **API Core Spring Boot 5 CI_JAVA_READY** (2026-07-15) — `.github/workflows/api-spring-ci.yml` (L5 : Java 21 Temurin + Maven Wrapper + `./mvnw verify` + Testcontainers PostgreSQL, Docker `ubuntu-latest`, FakeStorageService) ; scope `api-spring` dans `quality-gates.mjs` (8ème scope, `./mvnw verify --no-transfer-progress`, exclu de `all-safe`) ; tests 36→**42/42** ; `QUALITY_GATES_MATRIX.md` L5 + §2.9 + check recommandé §3. Non livré : liste/delete/download/URL signée, MinIO TC, Tika, quarantaine. **API Core Spring Boot 6 V1 Readiness Review** (2026-07-15) — rapport `API_SPRING_V1_READINESS_REVIEW.md` ; §30 11/15 ✅ / 3/15 ⚠️ / 1/15 ✗ ; B1 audit logs absent (module obligatoire §9), B2 URL signée absente (§20) ; 5 réserves acceptées (R1 MinIO TC, R2 CORS env var, R3 rate limiting, R4 Tika, R5 Redis) ; `api-spring` **`CI_JAVA_READY` → `IMPLEMENTATION_AVANCEE`**. **API Core Spring Boot 7** (2026-07-15) — SB7 : B1 AuditModule (`V3__add_audit_logs.sql`, 7 events, REQUIRES_NEW, aucun payload sensible), B2 URL signée (presigned TTL 300s, no-store, anti-énumération 404), C15 CORS (`${CORS_ALLOWED_ORIGINS}`) ; **90/90 ✅** ; `IMPLEMENTATION_AVANCEE` → `VALIDE_V1`. **API Core Spring Boot 8** (2026-07-15) — SB8 : `spring-boot-starter-data-redis` + `${REDIS_URL}` ; `RateLimitInterceptor` fixed-window en mémoire (`@ConditionalOnProperty`, 4 endpoints, 429 `ApiError`) ; `RateLimitConfig` externalisé ; `MinioStorageIntegrationTest` TC minio réel (`@TestConfiguration @Primary StorageService`, upload → `listObjects`, URL `X-Amz-*`, no-store) ; `RedisHealthIntegrationTest` TC redis:7-alpine (redis UP + db UP) ; C10/R1/R3/R5 fermés ; **99/99 ✅ BUILD SUCCESS** ; §30 **15/15 ✅**. |
...
> **Mise à jour Mobile Core V1 final readiness decision** (2026-07-13) : le core
> `mobile-react-native` passe de **`IMPLEMENTATION_AVANCEE`** à **`VALIDE_V1`**.
> Rapport : `docs/project-status/MOBILE_CORE_V1_FINAL_READINESS_DECISION.md`.
> B2 iOS est acceptée comme réserve environnementale documentée : aucun smoke iOS réel
> n'est revendiqué, aucun succès artificiel n'est créé.

```
enistere-os-foundation/
  strategy/            10 docs Phase 0 (01..10)
  docs/
    adr/               22 ADR (001–016, 034, 035, 039, 040, 041, 042) + ADR_BACKLOG + ADR_V1_BLOCKING_REVIEW
    project-status/    CE checkpoint (source de pilotage officielle)
    project-factory/   SPECIFICATION_DOCUMENTAIRE (processus projets derives + matrice profils stack)
  ...
  cores/
    ...
    ui-kit/            VALIDE_V1 (tokens + 19 primitives Web React, React 19, 181 tests ; §12.4 4/4 + §59 9/9) — v0.1.1
    ...
    cloud/             VALIDE_V1 (CI runtime API + E2E navigateur + registry GHCR + CC10 staging HTTPS réel + CC11 socle opérationnel ; Redis reporté V2, CC10 compose V1 officiel)
    ...
    ai-core/               VALIDE_V1 (AI Core V1 Final Readiness Decision — §19 8/8 ; B1/B2/B3 fermés)
  ...
  packages/
    api-contracts/     @enistere/api-contracts (0.1.0, publish-ready, non publié)
    api-client-fetch/  @enistere/api-client-fetch (0.1.0, publish-ready, non publié)
  package.json         racine privé, workspaces ["packages/*","packages/ui-kit","starters/nextjs"]
  factory/ai/prompts/ templates/  présents ; examples/api-client-node/ présent ; tools/ vide
...
|---|---|---|---|---|
| `api-nestjs` | oui | oui | **oui** | **VALIDE_V1** |
| `ui-kit` | oui | oui | **oui** (tokens + 19 primitives Web, React 19 ; états UI UI Kit 6 ; 181 tests) | **VALIDE_V1** |
...
| `ai-core` | oui | oui (`CORE_SPECIFICATION.md`, `README.md`, `AI_CORE_USAGE_RUNBOOK.md`) | **oui** (`prompt-registry.json`, validateur Node pur, redaction layer, context builder, provider fake, evaluation harness, décision Retrieval/RAG V1, schema rapport d'execution, runner prompt fake-provider, citations retrieval, runbook usage) | **VALIDE_V1** |
...
| `web-angular` | oui | **oui** (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15) | **oui** (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 Reactive Forms + Angular Material + Angular 6 Foundation state components + Angular 7 Upload fichiers + Angular 8 Tests + smoke + Angular 9 RefreshInterceptor/login API seam + Angular 10 PermissionService/PermissionDirective ; 267/267 tests) | **VALIDE_V1** (Angular 10, 2026-07-16 — 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; zéro blocker V1) |
...
`STRUCTURED_LOGGING_COMPATIBILITY_PROOF`. Détail : [`../../starters/nestjs/docs/API_CORE_V1_IMPLEMENTATION_STATUS.md`](../../starters/nestjs/docs/API_CORE_V1_IMPLEMENTATION_STATUS.md).

## 5. Packages

| Package | Version | Privé | Build/Tests | Publié | Intégré dans un core |
|---|---|---|---|---|---|
| `@enistere/api-contracts` | 0.1.0 | non | oui (types-only, 12 tests + `pack:dry-run`) | **non** | **consommé (types) dans `web-nextjs`** (Health + Auth + **Files** : `PublicStoredFileDto`/`SignedDownloadResponseDto` via `SchemaOf<>`) |
| `@enistere/api-client-fetch` | 0.1.0 | non | oui (30 tests + live 16/16 + `pack:dry-run`) | **non** | **instancié (public/Health + authentifié/BFF Auth + façade Files lecture) dans `web-nextjs`** |
...
**Distribution packages (2026-07-17/18)** : la stratégie est documentée dans
[`PACKAGES_DISTRIBUTION_DECISION.md`](./PACKAGES_DISTRIBUTION_DECISION.md). Cible principale :
GitHub Packages npm registry pour `@enistere/*`. Repli : GitHub Release tarballs. La préparation
publish-ready est livrée dans [`PACKAGES_DISTRIBUTION_2_PUBLISH_READY_REPORT.md`](./PACKAGES_DISTRIBUTION_2_PUBLISH_READY_REPORT.md)
(`private:false`, `publishConfig`, SemVer interne `0.1.0`, `npm pack --dry-run`). La première distribution
est exécutée par repli GitHub Release tarballs : tag `packages-api-typescript-v0.1.0`, assets
`enistere-api-contracts-0.1.0.tgz` et `enistere-api-client-fetch-0.1.0.tgz`. Rapport :
[`PACKAGES_RELEASE_1_TARBALLS_REPORT.md`](./PACKAGES_RELEASE_1_TARBALLS_REPORT.md).

**Exemple consommateur (2026-07-17)** : `examples/api-client-node` fournit un smoke Node sans backend
(`fetch` mocke) qui consomme le `dist` local de `@enistere/api-client-fetch`, prouve un appel public
`/health` sans `Authorization` et un appel authentifie `/auth/me` via `InMemorySessionAdapter`.
Commande : `npm run example:api-client-node`. Rapport :
[`EXAMPLES_CORE1_API_CLIENT_NODE_REPORT.md`](./EXAMPLES_CORE1_API_CLIENT_NODE_REPORT.md).

**Quickstart développeur (2026-07-17)** : `docs/onboarding/DEVELOPER_QUICKSTART.md` fournit un parcours
15 minutes : verification Git, lecture minimale des sources de verite, `quality-gates docs`, exemple
`api-client-node` et choix de mission via `NEXT_ACTIONS.md`. Rapport :
[`DEVELOPER_QUICKSTART1_REPORT.md`](./DEVELOPER_QUICKSTART1_REPORT.md).
...
**22 ADR rédigés et Validés** : ADR-001..016, ADR-034, **ADR-035** (Angular UI stack, 2026-07-15), ADR-039, ADR-040, **ADR-041** (build system Maven, 2026-07-14), **ADR-042** (Project Factory AI-native, 2026-07-18). ADR-017→033 et ADR-036→038 sont **listés dans
...
preuve live **16/16** (client officiel vs API réelle). UI Kit : **181 tests** (`node:test` + `global-jsdom`
Testing Library + jest-axe, **React 19**) couvrant **19 primitives** (tokens, composants de base,
feedback, data et états UI).
Web Core : **450 tests** (`node:test` :
...
- `OPENAPI_CLIENT_PROOF.md` — preuve `openapi-typescript`/`openapi-fetch` (concluante, **migrée** en
  packages ; code de preuve retiré, voir `starters/nestjs/proofs/openapi-client/README.md`).
...
Puis le **Cloud Core 6 — déploiement staging manuel** a livré le **cadrage** staging (`deployment/core/staging/` :
...
l'image)** ; décision **MinIO/URL signée** = Option A. Détail : `deployment/core/docs/STAGING_DRY_RUN_REPORT.md` §8.
...
staging → **`EXECUTION_LOCALE_CONTROLEE`** (détail : `deployment/core/docs/STAGING_EXECUTION_REPORT.md`). Enfin, une
...
**Actions humaines restantes** : activer les deux checks `images` comme requis dans `protect-main` si la
recommandation Quality Core est acceptée.
...
6. **Image runtime API — défaut Prisma engine CORRIGÉ (Cloud Core 8)** : le query engine de `.prisma/client`
   ...
   faire (humain) : rendre **`api-smoke` requis** sur `main`. Détail : `deployment/core/docs/STAGING_DRY_RUN_REPORT.md` §8.
...
([`WEB_CORE_GOVERNANCE_REVIEW.md`](../../starters/nextjs/docs/WEB_CORE_GOVERNANCE_REVIEW.md)), **Web Auth 4** a
...
[`protected-routes.md`](../../starters/nextjs/docs/protected-routes.md),
[`login-flow.md`](../../starters/nextjs/docs/login-flow.md)). La **Revue globale Auth Web (1 → 5)** a été
**réalisée** (rapport [`WEB_AUTH_V1_REVIEW.md`](../../starters/nextjs/docs/WEB_AUTH_V1_REVIEW.md)) — verdict
...
structurels** : primitives UI Kit `Alert`/`Card`/`FormField` et états UI (socle UI Kit désormais **181 tests**) + compositions Web
...
sensible** (détail [`ui-states.md`](../../starters/nextjs/docs/ui-states.md)). Enfin **Web Core Files 1** a livré
...
[`WEB_CORE_V1_INCREMENT_REVIEW.md`](../../starters/nextjs/docs/WEB_CORE_V1_INCREMENT_REVIEW.md)) — socle **sûr
...
Enfin le **Cloud Core 1 — cadrage d'exécution CI/CD & environnements** a été **réalisé** (`deployment/core/docs/` +
`deployment/core/README.md`) : baseline d'exécution (17 sections), environnements logiques, **checklist de
...
**Prochaine action** : Quality/Governance — gate CI `web-angular` dédié, ou prochain core prioritaire de la roadmap.
