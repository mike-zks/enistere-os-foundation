...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  LOCAL_STATE_READY (Expo SDK 55 + Expo Router : app/ + src/{api,auth,config,forms,navigation,offline,query,states,store,storage,theme,types,ui} + metro.config.js + test/ node --test ; AuthEngine + SessionStore + 401-refresh ; forms RHF+Zod ; offline queue mémoire ; client OFFICIEL @enistere/api-client-fetch lié file: + pont 401 authedRequest ; couche server-state TanStack Query générique ; état local Zustand (useUiStore, séparé) + purge logout déterministe câblée ; autonome hors workspaces)
...
**RN 4/4B — client officiel + pont 401**, **RN 5 — server-state**, puis **RN 6 — état local UI + purge logout
RÉALISÉ** (`mobile-react-native` → **LOCAL_STATE_READY** ; **Zustand** `useUiStore` générique (primitives UI non
sensibles : `themePreference` + `flags` booléens) **séparé du server-state** (anti-pattern spec §57), **in-memory
sans persistance** ; **purge logout déterministe câblée** dans `AuthProvider` : `await cancelQueries`→`clear` dès
`unauthenticated`/`expired` (signOut + expiry), **AuthEngine inchangé** ; **67 tests `node --test`**, typecheck/
lint/test verts ; *(garde CI `npm ls zustand` au root inchangée — mobile autonome, hors scope)*). Cloud Core reste
**PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 7 —
upload sécurisé (multipart)** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (LOCAL_STATE_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout**) ≠ implémentation complète (V1 partielle : upload, notifications, logger, offline sync réelle — différés).
