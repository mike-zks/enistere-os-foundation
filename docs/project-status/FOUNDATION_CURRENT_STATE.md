...
succès iOS artificiel. `web-angular` est **`TEST_SMOKE_READY`** (Angular 8,
2026-07-16 — 16 tests ajoutés : `RouterTestingHarness` (5 tests intégration router lazy-loaded, guards, redirect) + CDK harnesses (11 tests : `MatFormFieldHarness`/`MatInputHarness` login, `MatSelectHarness`/`MatButtonHarness`/`MatProgressSpinnerHarness` upload+loading) ; build SUCCESS + 224/224 tests). Le seul core encore vide est `ai-core`.
...
| Cores vides | `ai-core` ; `web-angular` passe à **`TEST_SMOKE_READY`** (Angular 8, 2026-07-16 — 16 tests ajoutés : `RouterTestingHarness` (5 tests intégration router lazy-loaded, guards, redirect) + CDK harnesses login/upload/loading (11 tests) ; 224/224 tests ✅ ; prochaine action : Web Core Angular V1 Readiness Review) |
