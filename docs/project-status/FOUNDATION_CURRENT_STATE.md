...
| Cores documentaires | **`quality-core`** — **SPECIFICATION_DOCUMENTAIRE** (Quality Core 3, 2026-07-11) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md` (matrice 8 cores × 11 gates), `BRANCH_PROTECTION_RUNBOOK.md` (**procédure activation protection branche `main`** — 10 noms de checks exacts, options recommandées, checklist post-activation). Checklists : `docs/checklists/` (3). Scripts : `scripts/quality-gates.mjs` (7 scopes) + `scripts/quality-gates.test.mjs` (**36/36 tests**). **Protection branche `main` : documentée, non appliquée** (action humaine requise). Aucun workflow modifié, aucune dépendance, aucun changement runtime. _(aucun autre core documentaire ; `mobile-react-native` est passé au starter ci-dessus)_ |
