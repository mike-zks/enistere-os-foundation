...
| Quality Core | **`quality-core`** — **IMPLEMENTATION_AVANCEE** (Quality Core Advanced Readiness Review, 2026-07-12) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md`, `BRANCH_PROTECTION_RUNBOOK.md`, **`RELEASE_PROCESS_RUNBOOK.md`**, **`AI_PROMPT_GOVERNANCE.md`**, templates GitHub PR/Issues, 3 checklists, `scripts/quality-gates.mjs` + tests (**36/36**) ; prompts catalogués ; processus utilisé pour publier `foundation-v1.0.0` ; gates documentaires consommes par Docs Core. **Protection `main` active via GitHub Rulesets** (`protect-main`, enforcement `active`, 8 checks requis). Aucun workflow modifié, aucune dépendance, aucun changement runtime. |
...
enistere-os-foundation/
  ...
  cores/
    ...
    quality-core/      IMPLEMENTATION_AVANCEE (gates, checklists, runbooks, templates, prompts, release process utilisé)
...
| `quality-core` | oui | oui | **oui** (gouvernance qualité : gates, checklists, runbooks, templates, prompts, release process utilisé ; 36 tests script) | **IMPLEMENTATION_AVANCEE** |
