...
**RN 11 — i18n / localisation primitives génériques**, **RN 12 — deep-linking / routing primitives génériques**, puis
**RN 13 — analytics / télémétrie primitives génériques (avec redaction, sans SDK réel) RÉALISÉ** (`mobile-react-native`
→ **ANALYTICS_READY** ; `AnalyticsEvent` borné aux primitives + **redaction dédiée mais BASÉE sur RN 8** : `isSensitiveProperty`
**réutilise `isSensitiveKey`** + couche normalisée exact/substring (durcissement RN 12), `sanitizeAnalyticsEvent`
**supprime les clés sensibles** + **scrube les valeurs via `redactString`** + **borne** (jamais de throw) ;
`AnalyticsAdapter` (`track`/`flush?`, **PAS de `identify`** — pas d'user-id réel) + `createAnalyticsService` (track
**best-effort non-intrusif** — ne casse jamais le flux app, **logs RN 8 sûrs** `{eventName,propertyCount}` — jamais les
valeurs, erreurs adapter **contrôlées** sans cause sensible) ; **placeholder** mémoire (tests) ; **aucun SDK réel**
(Sentry/Amplitude/GA/…), **aucun réseau/persistance/identité utilisateur réelle/token** ; **175 tests `node --test`**,
typecheck/lint/test/doctor verts ; RN 13 **n'ajoute aucune dépendance**). Cloud Core reste **PAUSE_CONTROLEE**, staging
**EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 14 — accessibilité (a11y) primitives
génériques** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (ANALYTICS_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales + i18n/localisation + deep-linking/routing + analytics/télémétrie**) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs Expo réels (permissions/notifications/localisation/linking), catalogues métier i18n + routes concrètes, SDK analytics réel, backend d'observabilité, offline sync réelle — différés).
