...
succès iOS artificiel. Les cores vides restent `ai-core` et `web-angular`. `api-spring` est **`VALIDE_V1`** (Spring Boot 7 — SB7 AuditModule + URL signée + CORS env var, 2026-07-15 : §30 14/15 ✅ / 1/15 ⚠️ (C10 Redis différé) / 0 ✗ ; B1 audit logs ✅ fermé (SB7) ; B2 URL signée ✅ fermée (SB7) ; C15 CORS env var ✅ fermée (SB7) ; `./mvnw verify` 89/89 ✅ ; CI L5 active).
...
enistere-os-foundation/
  ...
  cores/
    ...
    api-spring/           VALIDE_V1 (SB7 AuditModule + URL signée + CORS env var 2026-07-15 — 14/15 §30 ✅ ; 89/89 ✅ ; L5 CI)
...
| `api-spring` | oui | oui | **oui** (Spring Boot 1→7 — JPA + Flyway V1+V2+V3 + Argon2id + RBAC + OpenAPI + upload + URL signée + AuditModule + CORS env var + FakeStorageService ; 89/89 ✅ ; CI L5) | **VALIDE_V1** (SB7, 2026-07-15 — 14/15 §30 ✅ ; B1+B2+C15 fermés ; R5 Redis différé) |
