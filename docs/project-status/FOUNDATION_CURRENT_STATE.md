...
succès iOS artificiel. `web-angular` est **`UPLOAD_READY`** (Angular 7,
2026-07-16 — `FileCategory` (9 valeurs) + `AppFile` + `UploadService` (HttpClient + FormData, Content-Type jamais forcé, 413→`FileTooLarge`/415→`UnsupportedType`/401→`Unauthorized` via `AppApiError`) + `UploadFormComponent` (Reactive Forms + Material, états Loading/Error/Success via composants Foundation Angular 6) ; build SUCCESS + 205/205 tests). Le seul core encore vide est `ai-core`.
...
| Cores vides | `ai-core` ; `web-angular` passe à **`UPLOAD_READY`** (Angular 7, 2026-07-16 — `FileCategory` + `AppFile` + `UploadService` (HttpClient+FormData, Content-Type jamais forcé, 413/415/401→`AppApiError`) + `UploadFormComponent` (Reactive Forms+Material, états Foundation) ; 205/205 tests ✅ ; prochaine action : Web Core Angular 8) |
