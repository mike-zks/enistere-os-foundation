...
| Core mobile (socle) | **`mobile-react-native`** — **STARTER_EXPO_DOCTOR_GREEN** : socle Expo SDK 55 / Expo Router RN 1→34. Primitives RN 1→25 inchangées, Settings protégé RN26, shell durci RN27, smoke visuel Android RN28, smoke runtime RN29, préflight iOS RN30 bloqué (Linux), RN31 en attente macOS/Xcode, RN32 formulaire sign-in générique RHF+Zod, RN33 câblage préférence thème, **RN34 alignement patch Expo SDK** (`expo` 55.0.27, `expo-linking` 55.0.16, `expo-secure-store` 55.0.15). Preuves : typecheck + lint + `npm test` 355/355 `node --test` + expo-doctor **19/19** + `expo export -p ios` + `npm audit` 0 vuln + `git diff --check` verts. `npm run smoke:android` **passed** (`emulator-5554` / Pixel_6a, 2026-07-08). `npm run smoke:ios` blocked (Linux). Aucun réseau métier, endpoint métier, SDK/adaptateur natif réel, nouvelle dépendance, retry branché ni changement Auth/Query. |
...
> **Mise à jour RN34** : le statut courant du core `mobile-react-native` est
> **`STARTER_EXPO_DOCTOR_GREEN`**. RN34 aligne les patchs Expo SDK 55 nécessaires
> (`expo` 55.0.26→55.0.27, `expo-linking` 55.0.15→55.0.16,
> `expo-secure-store` 55.0.14→55.0.15) pour ramener expo-doctor de 18/19 à
> **19/19**. Aucun changement de code runtime. Vérification locale : typecheck,
> lint, test 355/355, expo-doctor **19/19**, expo export -p ios, npm audit 0 vuln,
...
[`files-read-download.md`](../../cores/web-nextjs/docs/files-read-download.md)). Enfin **Web Core Files 2** a livré l'**upload sécurisé BFF multipart** : BFF ciblé `POST /api/files/upload` (CSRF/Origin obligatoires, validation fichier+catégorie, client `writable` avec un seul refresh coordonné), client BFF navigateur `uploadFile` (**FormData sans Content-Type forcé**, same-origin, aucun Bearer), mutation `useUploadFile` (**sans `mutationKey`**, anti-double-soumission, résultat jamais en QueryCache), `UploadForm` (9 catégories Select, fichier+subjectId, Alert erreur/succès), page `/protected/files/upload`, mapping 413/415 — **l'API Core reste l'autorité** MIME/taille/permissions (ADR-007), **aucun upload direct MinIO/S3**, **aucun log de nom/contenu** (**333 tests**). Enfin la **Revue globale Web
