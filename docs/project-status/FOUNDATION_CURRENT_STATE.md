...
**RN 11 — i18n / localisation primitives génériques**, puis **RN 12 — deep-linking / routing primitives génériques
RÉALISÉ** (`mobile-react-native` → **LINKING_READY** ; parseur pur (`parseDeepLink`/`decodeSafe`/`normalizeUrl`, custom
schemes + `https`, **sans `URL` global** → déterministe) + **`resolveLink`** (`internal`/`externalBlocked`/`invalid`) —
**allowlist stricte** schemes/hosts (**`http` → `insecure_scheme`**), **anti-open-redirect** (route encodant
`//authority` ou `scheme://` → bloquée ; traversal `..`/`.` → `unsafe_path`), **params sensibles supprimés** (jamais
conservés, aucune URL complète gardée), **bornes** ; `isInternalRoute` ; **`resolveNotificationLink`** (clé configurable,
tap notification RN 10) ; **aucun log** (ni query sensible), **aucun stockage** de lien/URL, **aucune dépendance** ;
routes concrètes = projets dérivés ; **159 tests `node --test`**, typecheck/lint/test/doctor verts ; RN 12 **n'ajoute
aucune dépendance**). Cloud Core reste **PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** :
**Mobile Core React Native 13 — analytics / télémétrie primitives génériques (avec redaction, sans SDK réel)** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (LINKING_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales + i18n/localisation + deep-linking/routing**) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs Expo réels (permissions/notifications/localisation/linking), catalogues métier i18n + routes concrètes, backend d'observabilité, offline sync réelle — différés).
