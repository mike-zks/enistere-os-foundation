...
| Packages officiels | `@enistere/api-contracts` (**12 tests**), `@enistere/api-client-fetch` (**30 tests**) (**publish-ready**, non publiés ; **instanciés (public + authentifié/BFF)** dans le Web Core — preuve API réelle ; **`FilesApi.list()` ajouté (Files 5)**, `files_list` dans schema.ts ; **distribution décidée** le 2026-07-17 : GitHub Packages npm registry, repli GitHub Release tarballs ; **dry-runs `npm pack` validés**, aucune publication effectuée ; premier exemple consommateur executable `examples/api-client-node` livré ; quickstart développeur 15 minutes livré) |
...
**Distribution packages (2026-07-17)** : la stratégie est désormais documentée dans
...
(`private:false`, `publishConfig`, SemVer interne `0.1.0`, `npm pack --dry-run`). Les packages restent
non publiés jusqu'à une publication contrôlée ou l'attachement de tarballs à une GitHub Release.
