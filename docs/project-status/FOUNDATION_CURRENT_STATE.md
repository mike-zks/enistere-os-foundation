...
| Core mobile (socle) | **`mobile-react-native`** — **AUTH_SESSION_HARDENED** : socle **Expo SDK 55** / Expo Router durci (RN 2) — **AuthEngine** framework-agnostique (restore/signIn/signOut/refresh/clear, **refresh coalescé**, **expiration**), `SessionStore` SecureStore avec validation (access token **en mémoire**, refresh persistant), **API client `401` → refresh → retry** (ADR-011), gardes `expired`/`refreshing`, seam `@enistere/api-client-fetch` (ADR-016) ; **21 tests `node --test`** ; **typecheck + lint + test 21/21 + expo-doctor 19/19 verts** ; **aucune logique métier** |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-react-native/  AUTH_SESSION_HARDENED (Expo SDK 55 + Expo Router : app/ + src/{api,auth,config,navigation,query,states,storage,theme,types,ui} + test/ node --test ; AuthEngine + SessionStore + 401-refresh ; autonome hors workspaces)
...
| `mobile-react-native` | oui | oui | **oui** (Expo SDK 55 + Expo Router : navigation auth/privé durcie, AuthEngine + refresh coalescé + expiration, SessionStore SecureStore, API client 401→refresh→retry, ThemeProvider, états ; 21 tests) | **AUTH_SESSION_HARDENED** |
...
**reporté** — dépendance externe), **retour aux priorités V1**. **Mobile Core RN 1 (starter, #11 mergé)** puis
**Mobile Core RN 2 — auth/session hardening RÉALISÉ** (`mobile-react-native` → **AUTH_SESSION_HARDENED** ; AuthEngine
+ refresh coalescé + expiration + API client 401→refresh, 21 tests `node --test`, typecheck/lint/doctor verts). Cloud
Core reste **PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React
Native 3 — forms, validation and offline-ready primitives** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (AUTH_SESSION_HARDENED) ≠ implémentation complète (V1 partielle : Zustand/forms/upload/notifications différés).
