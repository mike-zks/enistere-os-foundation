...
| Cores documentaires | **`quality-core`** — **SPECIFICATION_DOCUMENTAIRE** (Quality Core 4, 2026-07-11) : `CORE_SPECIFICATION.md`, `README.md`, `QUALITY_GATES_MATRIX.md` (matrice 8 cores × 11 gates), `BRANCH_PROTECTION_RUNBOOK.md` (10 noms de checks exacts, options recommandées, checklist post-activation). **Templates GitHub** (QC4) : `.github/PULL_REQUEST_TEMPLATE.md` (sections gates, scope, hors périmètre, sécurité, gouvernance), `.github/ISSUE_TEMPLATE/` (bug / feature / security alignés QC). Checklists : `docs/checklists/` (3). Scripts : `scripts/quality-gates.mjs` (7 scopes) + `scripts/quality-gates.test.mjs` (**36/36 tests**). **Protection branche `main` : documentée, non appliquée** (action humaine requise). Aucun workflow modifié, aucune dépendance, aucun changement runtime. _(aucun autre core documentaire ; `mobile-react-native` est passé au starter ci-dessus)_ |
