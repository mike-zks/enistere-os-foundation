...
B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; build SUCCESS + 267/267 tests). `ai-core` est désormais **`PREUVE_TECHNIQUE`** (AI Core 3, 2026-07-16 — registre de prompts local + validateur Node pur + redaction layer pure ; aucun provider, SDK IA, RAG runtime, endpoint, secret, dépendance ou réseau).
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               PREUVE_TECHNIQUE (AI Core 3 — prompt registry + redaction layer, aucun provider/runtime)
...
| `ai-core` | oui | oui (`CORE_SPECIFICATION.md`, `README.md`) | **oui** (`prompt-registry.json`, validateur Node pur, redaction layer) | **PREUVE_TECHNIQUE** |
