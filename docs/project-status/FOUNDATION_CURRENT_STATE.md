...
primitives génériques**, **RN 15 — app lifecycle primitives génériques**, puis **RN 16 — connectivité réseau (network
status) primitives génériques RÉALISÉ** (`mobile-react-native` → **NETWORK_STATUS_READY** ; **étend `src/offline`** —
RN 3 inchangé, **`shouldQueueMutations` reste l'API canonique** (queue sauf positivement `online`) ; ajouts :
`NetworkConnectionType` **borné** (jamais SSID/carrier/IP) + `type?` optionnel sur `NetworkState` + `NetworkSnapshot` +
**`normalizeNetworkStatus`**/**`normalizeConnectionType`** (tolérants) ; `NetworkAdapter` (seam RN NetInfo) +
**`NetworkAdapterError`** contrôlé + **placeholder** mémoire + `createNetworkService` (`getStatus(): NetworkState`/
`shouldQueue`/`subscribe`/`transition`/`dispose`, **`changedAt` stampé sur changement de STATUS** via **horloge
injectée** ; **best-effort non-intrusif** — erreurs adapter **capturées** + `warn` sûr, défaut `unknown`, **listener qui
throw isolé** ; **logs RN 8 sûrs** `{from,to,type}`/`{operation}` enums — **aucune donnée device/PII**) ; **aucun NetInfo
réel/dépendance/offline sync/persistance/donnée sensible** ; **227 tests `node --test`** (dont `network-state` RN 3
inchangé), typecheck/lint/test/doctor + `git diff --check` verts ; RN 16 **n'ajoute aucune dépendance**). Cloud Core
reste **PAUSE_CONTROLEE**, staging **EXECUTION_LOCALE_CONTROLEE**. **Prochaine action** : **Mobile Core React Native 17 —
feature flags / config primitives génériques** ; **actions humaines** : protection de branche `main` + rendre `api-smoke` requis.
...
3. **Spécifications sans starter** — `cloud` peut être lu à tort comme implémenté (PARTIEL/PAUSE). `mobile-react-native` dispose d'un socle durci (NETWORK_STATUS_READY : auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales + i18n/localisation + deep-linking/routing + analytics/télémétrie + accessibilité a11y + app lifecycle + connectivité réseau**) ≠ implémentation complète (V1 partielle : écran/picker d'upload, push distant réel + token device, adaptateurs natifs réels (permissions/notifications/localisation/linking/AccessibilityInfo/AppState/NetInfo), catalogues métier i18n + routes concrètes, SDK analytics réel, application des props a11y dans des composants, câblage des effets lifecycle, offline sync réelle, backend d'observabilité — différés).
