...
modele de composition des cores + cinq scenarios de preuve + roadmap d'adoption post-V1.
Les profils API + mobile (`nestjs-react-native`, `spring-flutter`, `spring-react-native`, `nestjs-flutter`)
sont explicitement reconnus. Les cores sont composes par package/bootstrap/reference/adapter/deploy profile,
pas fusionnes physiquement ; `VALIDE_V1` signifie starter gouverne et non vision finale complete. Aucun
generateur CLI, runtime applicatif ou exemple derive bout-en-bout n'est encore livré.

| Catégorie | État |
|---|---|
| Stratégie (Phase 0) | 10 documents présents |
| ADR | **21 ADR rédigés et Validés** (001–016, **034**, **035**, 039, 040, **041**) ; ADR-017→033 et ADR-036→038 = backlog non rédigé |
...
| Project Factory | **`SPECIFICATION_DOCUMENTAIRE`** — processus + profils + templates + modèle de composition + scénarios d'usage + roadmap post-V1 ; aucun générateur/runtime dérivé |
...
enistere-os-foundation/
  ...
  docs/
    adr/               21 ADR (001–016, 034, 035, 039, 040, 041) + ADR_BACKLOG + ADR_V1_BLOCKING_REVIEW
  ...
  prompts/ templates/  présents ; examples/api-client-node/ présent ; tools/ vide
...
**21 ADR rédigés et Validés** : ADR-001..016, ADR-034, **ADR-035** (Angular UI stack, 2026-07-15), ADR-039, ADR-040, **ADR-041** (build system Maven, 2026-07-14). ADR-017→033 et ADR-036→038 sont **listés dans
