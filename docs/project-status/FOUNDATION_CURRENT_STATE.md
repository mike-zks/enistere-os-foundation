...
| Mobile Core Flutter | **`mobile-flutter`** — **`IMPLEMENTATION_AVANCEE`** (Flutter V1 Readiness Review, 2026-07-14) : §29 5/11 satisfaits (navigation, upload, thème, tests, API Dio). Bloquants V1 : B1 Android runtime (library sans `android/`), B2 `flutter_secure_storage` absent, B3 `RefreshInterceptor` absent, B4 UI states absents, B5 login form absent. Réserves acceptées : R1 iOS Linux, R2 pas de backend réel, R3 Freezed délibérément absent. Rapport : `MOBILE_FLUTTER_V1_READINESS_REVIEW.md`. Prochain : Flutter 7 — platform dirs + smoke Android. |
...
| V3 | **En cours** — ADR-034 validé ; Flutter 1 spec ✅ ; Flutter 2 starter ✅ ; Flutter 3 auth shell ✅ ; Flutter 4 Dio client ✅ ; Flutter 5 upload multipart ✅ ; Flutter 6 tests + smoke ✅ ; Flutter V1 Readiness Review ✅ (`IMPLEMENTATION_AVANCEE`) ; prochaine : Flutter 7 — platform dirs + smoke Android |
...
enistere-os-foundation/
  ...
  cores/
    ...
    mobile-flutter/      IMPLEMENTATION_AVANCEE (Flutter V1 Readiness Review — §29 5/11, B1→B5 bloquants, Flutter 7→11 vers VALIDE_V1)
...
| `mobile-flutter` | oui | oui | oui (starter + auth shell + Dio client + upload primitives + widget/integration tests) | **IMPLEMENTATION_AVANCEE** (Flutter V1 Readiness Review, 2026-07-14 — §29 5/11 ; B1→B5 bloquants ; rapport `MOBILE_FLUTTER_V1_READINESS_REVIEW.md`) |
