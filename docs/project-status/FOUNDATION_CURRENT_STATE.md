...
succès iOS artificiel. `web-angular` est **`FOUNDATION_STATES_READY`** (Angular 6,
2026-07-16 — 4 composants Foundation standalone `EnistereLoadingStateComponent`/`EnistereEmptyStateComponent`/`EnistereErrorStateComponent`/`EnistereSuccessStateComponent` : signal inputs typés, `role="status"`/`"alert"`, `aria-live="polite"`/`"assertive"`, tokens Enistere, `MatProgressSpinnerModule`/`MatButtonModule` ; construit sur Angular 5 Reactive Forms+Material ; build SUCCESS + 170/170 tests). Le seul core encore vide est `ai-core`.
...
| Cores vides | `ai-core` ; `web-angular` passe à **`FOUNDATION_STATES_READY`** (Angular 6, 2026-07-16 — 4 composants Foundation `EnistereLoadingState`/`EnistereEmptyState`/`EnistereErrorState`/`EnistereSuccessState` standalone, signal inputs, a11y roles (`role="status"`/`"alert"`, `aria-live`), tokens Enistere, `MatProgressSpinnerModule`/`MatButtonModule` ; 170/170 tests ✅ ; prochaine action : Web Core Angular 7) |
...
enistere-os-foundation/
  ...
  cores/
    ...
    ai-core/               → vide
    web-angular/           FOUNDATION_STATES_READY (Angular 6, 2026-07-16 — 4 composants Foundation standalone LoadingState/EmptyState/ErrorState/SuccessState ; signal inputs ; a11y roles ; tokens Enistere ; 170/170 tests)
  packages/
...
| `web-angular` | oui | **oui** (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15) | **oui** (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 Reactive Forms + Angular Material + Angular 6 Foundation state components — `EnistereLoadingState`/`EnistereEmptyState`/`EnistereErrorState`/`EnistereSuccessState`, signal inputs, a11y roles, tokens Enistere) | **FOUNDATION_STATES_READY** |
