...
- **Vides** : `ai-core`, `web-angular` — **ADR-035 validé (2026-07-15)** : Angular Material (CDK + M3) contrôlé par tokens Enistere + composants maison. Blocker UI levé ; prochaine action : Web Core Angular 1 — Core specification.
...
**V3 ADR-035 — Angular UI stack decision** (`docs/adr/`, `docs/project-status/`) (2026-07-15) : ADR `docs/adr/ADR-035-angular-ui-material-vs-primeng.md` rédigé et validé. Décision : **Angular Material (CDK + Material 3) contrôlé par tokens Enistere + composants maison ciblés** pour le futur Web Core Angular. Angular CDK = couche comportementale/a11y (FocusTrap/LiveAnnouncer/FocusMonitor/ListKeyManager) ; tokens Enistere via `mat.define-theme()` + `--mat-*` CSS custom properties ; composants maison Enistere Angular (LoadingState/EmptyState/ErrorState/SuccessState) ; Reactive Forms obligatoire ; `@angular/cdk/testing` ; Angular Signals préféré, NgRx différé ; pas de PrimeNG, pas de shadcn/Radix côté Angular. ADR-016 §F (adaptateur OpenAPI Angular) décidé par preuve dans Web Core Angular 1. Aucun starter, aucune dépendance npm, aucun runtime. `web-angular` reste `DOSSIER_SEULEMENT`. Blocker UI levé. `quality-gates docs` 2/2 ✅ · `git diff --check` ✅. Mises à jour documentaires : ADR_BACKLOG.md + DECISIONS_REGISTER.md (21 ADR validés) + FOUNDATION_CURRENT_STATE.md + IMPLEMENTATION_MATRIX.md + NEXT_ACTIONS.md + SESSION_HANDOFF.md + CHANGELOG.md. **Prochaine action : Web Core Angular 1 — Core specification**.
