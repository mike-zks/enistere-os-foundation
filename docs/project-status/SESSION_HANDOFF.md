...
- **Vides** : aucun. **`ai-core`** → **`PREUVE_TECHNIQUE`** (AI Core 4, 2026-07-16) : `CORE_SPECIFICATION.md` + `README.md` + registre local `prompt-registry.json` + validateur Node pur (`src/prompt-registry/*`, `scripts/validate-prompt-registry.mjs`, tests) + redaction layer pure (`src/redaction/*`, tests) + context builder allow-list (`src/context/*`, tests) ; 8 prompts gouvernés validés ; redaction des clés sensibles, credentials, JWT, URLs signées, `.env`, clés privées, emails, chemins locaux, erreurs/cycles/profondeur ; builder local avec allow-list explicite, refus `.env`/deps/build, redaction avant sortie, rapport included/skipped/missing/truncated ; aucun runtime, provider, SDK IA, RAG, base vectorielle, endpoint, dépendance, secret ou appel réseau. **`web-angular`** → **`VALIDE_V1`** (Angular 10 + Quality Web Angular CI, 2026-07-16) : B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale **14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT**. Réserve non-bloquante restante : R1 CDK a11y FocusTrap/ARIA HTML. R2 CI gate fermée par `web-angular-ci.yml` + scope `quality-gates web-angular`. **267/267 tests ✅**. Prochaine action recommandée : **AI Core 5 — Provider adapter seam + fake provider**.
...
Prochaine action recommandée : **AI Core 5 — Provider adapter seam + fake provider**.
