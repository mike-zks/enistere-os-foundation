...
- **Vides** : `ai-core`. **`web-angular`** → **`VALIDE_V1`** (Angular 10 + Quality Web Angular CI, 2026-07-16) : B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale **14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT**. Réserve non-bloquante restante : R1 CDK a11y FocusTrap/ARIA HTML. R2 CI gate fermée par `web-angular-ci.yml` + scope `quality-gates web-angular`. **267/267 tests ✅**. Rapports : `WEB_ANGULAR_V1_READINESS_REVIEW.md`, `WEB_ANGULAR9_REFRESH_INTERCEPTOR_REPORT.md`, `WEB_ANGULAR10_PERMISSION_RBAC_REPORT.md`, `QUALITY_WEB_ANGULAR_CI_GATE_REPORT.md`. Prochaine action recommandée : **AI Core 1 — Core specification**.
