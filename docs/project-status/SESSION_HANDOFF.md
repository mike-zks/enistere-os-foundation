...
**Quality Core 4 — alignement templates PR / Issues avec Quality Core** (`.github/`) (2026-07-11) :
modernisation des templates GitHub pour guider les contributeurs vers les gates Quality Core, les statuts de core et la sécurité. **`.github/PULL_REQUEST_TEMPLATE.md`** : sections Quality Gates (scope `docs`/`packages`/`ui-kit`/`web`/`mobile-static`/`root-audit`/`all-safe`/runtime, commandes exécutées, gates exclus), Hors périmètre confirmé, Sécurité renforcée, Statut / gouvernance (si project-status modifié). **`.github/ISSUE_TEMPLATE/`** : `bug_report.md` (environnement, core concerné, reproduction, impact sécurité, gate concerné), `feature_request.md` (core ciblé, roadmap, hors périmètre, critères d'acceptation), `security_issue.md` (canal privé si sensible, classification impact, scopes sensibles : auth/JWT/CSRF/URL signées/PII/staging/secrets), `config.yml` (Security Advisories). `CORE_SPECIFICATION.md` + `README.md` mis à jour. Aucun workflow GitHub modifié. Aucun secret. Aucune nouvelle dépendance. Vérifications : `git diff --check` ✓, `npm audit` 0 vuln ✓, `node --test cores/quality-core/scripts/quality-gates.test.mjs` 36/36 ✓.
