...
- **`VALIDE_V1`** : `api-spring` (Spring Boot 7 AuditModule + URL signée + CORS env var, 2026-07-15 — §30 **14/15 ✅ / 1 ⚠️ (C10 Redis) / 0 ✗** ; B1 ✅ fermé : `AuditModule` complet (`V3__add_audit_logs.sql` 8 colonnes 3 index, `AuditLog` + `AuditEventType` 7 valeurs + `AuditService` REQUIRES_NEW best-effort, traçage LOGIN_SUCCESS/FAILURE/LOGOUT/TOKEN_REFRESH/FILE_UPLOAD/FILE_DOWNLOAD_URL_CREATED/ADMIN_ACCESS, aucun payload sensible) ; B2 ✅ fermé : `GET /api/v1/files/{id}/download-url` ownership + 404 anti-énumération + presigned TTL 300s + `Cache-Control: no-store` ; C15 ✅ fermé : `CorsConfig @ConfigurationProperties` + `${CORS_ALLOWED_ORIGINS:...}` CSV robuste, wildcard ignoré avec credentials ; CI L5 **90/90 ✅ BUILD SUCCESS** ; rapport : `docs/project-status/API_SPRING_V1_READINESS_REVIEW.md`. Décision : **`IMPLEMENTATION_AVANCEE` → `VALIDE_V1`**.
...
**API Core Spring Boot 6 — V1 Readiness Review** (`docs/project-status/`) (2026-07-15) : revue complète §30 CORE_SPECIFICATION.md — 15 critères audités sur code source Java, migrations SQL, application.yml, CI L5 71/71 ✅. Résultat : **11/15 ✅ + 3/15 ⚠️ + 1/15 ✗**. Bloquants : **B1** — `AuditModule` (§9 module obligatoire) absent : pas de table `audit_logs` dans V1/V2, pas d'`AuditService`, pas d'`@Aspect` — les events `LOGIN_SUCCESS/FAILURE/LOGOUT/TOKEN_REFRESH/FILE_UPLOAD/ADMIN_ACCESS` ne sont pas tracés ; **B2** — URL signée (`GET /files/:id/download-url`, presigned URL §20) absente dans `FilesController`. Réserves acceptées : R1 MinIO TC (FakeStorageService intentionnel), R2 CORS hardcodé (dev local), R3 rate limiting différé, R4 Tika MIME différé, R5 Redis absent. Décision : `CI_JAVA_READY` → **`IMPLEMENTATION_AVANCEE`**. Rapport : `docs/project-status/API_SPRING_V1_READINESS_REVIEW.md`. Mise à jour : `IMPLEMENTATION_MATRIX.md` (SB6 entry), `FOUNDATION_CURRENT_STATE.md` (statut + §4), `NEXT_ACTIONS.md` (SB6 ✅ + SB7), `SESSION_HANDOFF.md`. Vérifications : `quality-gates docs` / `git diff --check` à exécuter après commit.
