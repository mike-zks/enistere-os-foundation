...
- **`SPECIFICATION_DOCUMENTAIRE`** : `api-spring` (Spring Boot 1, 2026-07-14 — `CORE_SPECIFICATION.md` 42 sections + `README.md` ; Spring Boot 4.x + Spring Security 7.x + JPA/Hibernate + PostgreSQL/Flyway + Redis + MinIO + springdoc-openapi + JUnit 5 + Testcontainers ; §30 critères V1 ; §41 missions ordonnées ; §42 cohérence NestJS ; §40 décisions pendantes Maven/Gradle ; aucun starter, aucun code Java). Prochain : Spring Boot 2 — Starter minimal.
