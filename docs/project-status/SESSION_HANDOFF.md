...
- **Vides** : `ai-core`. **`web-angular`** → **`IMPLEMENTATION_AVANCEE`** (Angular V1 Readiness Review, 2026-07-16) : 11/15 §29 SATISFAIT + 3/15 PARTIEL + 1/15 NON SATISFAIT. `VALIDE_V1` différé par B1 RefreshInterceptor/login API seam + B2 PermissionService/PermissionDirective. Réserves non-bloquantes : R1 CDK a11y FocusTrap/ARIA HTML, R2 CI gate/§32. 224/224 tests ✅. Rapport : `WEB_ANGULAR_V1_READINESS_REVIEW.md`. Prochaine action : **Web Core Angular 9 — RefreshInterceptor + login API seam**.
