...
- **Vides** : `ai-core`. **`web-angular`** → **`UPLOAD_READY`** (Angular 7, 2026-07-16) : `FileCategory` (9 valeurs : IMAGE/DOCUMENT/AVATAR/VIDEO/AUDIO/IDENTITY_DOCUMENT/ATTACHMENT/MEDIA/OTHER) + `AppFile {file, category, subjectId?}` + `isValidAppFile`/`isAllowedFileType`/`describeFileForLog({extension, sizeBytes})` + `UploadService` (HttpClient + FormData Content-Type jamais forcé, `createRequestState`, 413→`FileTooLarge`/415→`UnsupportedType`/401→`Unauthorized` via `errorInterceptor`+`AppApiError`) + `UploadFormComponent` (Reactive Forms `{category: required, subjectId: maxLength(128)}`, `signal<File|null>`, états Foundation Angular 6 pour loading/error/success). 205/205 tests ✅. Prochaine action : **Web Core Angular 8 — Tests + smoke**.
