...
- **Vides** : aucun. **`ai-core`** → **`IMPLEMENTATION_PARTIELLE`** (AI Core 7, 2026-07-16) : `CORE_SPECIFICATION.md` + `README.md` + registre local `prompt-registry.json` + validateur Node pur (`src/prompt-registry/*`, `scripts/validate-prompt-registry.mjs`, tests) + redaction layer pure (`src/redaction/*`, tests) + context builder allow-list (`src/context/*`, tests) + provider seam/fake provider (`src/provider/*`, tests) + evaluation harness local (`src/evaluation/*`, tests) + décision Retrieval/RAG V1 (`AI_CORE7_RETRIEVAL_RAG_DECISION.md`) ; 8 prompts gouvernés validés ; redaction des clés sensibles, credentials, JWT, URLs signées, `.env`, clés privées, emails, chemins locaux, erreurs/cycles/profondeur ; builder local avec allow-list explicite, refus `.env`/deps/build, redaction avant sortie ; fake provider déterministe sans réseau/modèle réel/stockage ; harness local vérifiant périmètre, docs requis, gates, format de rapport, preuve de vérification et secrets détectables ; Retrieval/RAG V1 = corpus Foundation versionné + allow-list explicite + revue humaine, sans embedding/vector DB/provider. Aucun provider réel, SDK IA, LLM judge, RAG runtime, base vectorielle, endpoint, dépendance, secret ou appel réseau. **`web-angular`** → **`VALIDE_V1`** (Angular 10 + Quality Web Angular CI, 2026-07-16) : B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale **14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT**. Réserve non-bloquante restante : R1 CDK a11y FocusTrap/ARIA HTML. R2 CI gate fermée par `web-angular-ci.yml` + scope `quality-gates web-angular`. **267/267 tests ✅**. Prochaine action recommandée : **AI Core 8 — Governance/execution report schema**.
...
Prochaine action recommandée : **AI Core 8 — Governance/execution report schema**.
...
Prochaine action : **AI Core 8 — Governance/execution report schema**.
