...
- **`SPECIFICATION_DOCUMENTAIRE`** : `api-spring` (Spring Boot 1, 2026-07-14 — `CORE_SPECIFICATION.md` 42 sections + `README.md` ; Spring Boot 4.x + Spring Security 7.x + JPA/Hibernate + PostgreSQL/Flyway + Redis + MinIO + springdoc-openapi + JUnit 5 + Testcontainers ; §30 critères V1 ; §41 missions ordonnées ; §42 cohérence NestJS ; **§40 — build system tranché par ADR-041 : Maven**). Spring Boot 2A (2026-07-14) — **ADR-041 Validé** : Maven comme build system V1 (`pom.xml`, Spring Boot Parent POM, `mvn verify`, Maven Wrapper) ; `docs/adr/ADR-041-build-system-api-spring-maven-vs-gradle.md`. Aucun starter, aucun code Java. Prochain : Spring Boot 2 — Starter minimal (`pom.xml` Maven + structure `src/` + Spring Security 7.x + JWT + auth flow).
