...
- **`IMPLEMENTATION_AVANCEE`** : `api-spring` (Spring Boot 6 V1 Readiness Review, 2026-07-15 — §30 11/15 ✅ ; B1 audit logs absent + B2 URL signée absente bloquent VALIDE_V1 ; CI L5 `api-spring-ci.yml` (Java 21 Temurin + Maven Wrapper + Testcontainers PostgreSQL, 71/71 ✅) ; `quality-gates api-spring` scope ajouté (SB5) — 8e scope exclu de `all-safe` ; auth complète (login/refresh/me/logout) + RBAC `@PreAuthorize` + Flyway V1+V2 + OpenAPI + DTO validation + ApiError + logs sûrs (ADR-040) + 71/71 tests. Bloquants : **B1** — `AuditModule` (§9 obligatoire) absent (pas de table `audit_logs`, pas de `AuditService`, pas d'`@Aspect`) ; **B2** — URL signée `GET /files/:id/download-url` absente (§20). CORS origines hardcodées (pas de `${CORS_ALLOWED_ORIGINS}`). Rapport : `docs/project-status/API_SPRING_V1_READINESS_REVIEW.md`. ADR-041 Maven. Prochain : **Spring Boot 7 — AuditModule + download URL signée + CORS env var**.
...
**Action unique suivante** : **API Core Spring Boot 7 — AuditModule + download URL signée + CORS env var** —
`AuditModule` : migration V3 `audit_logs`, `AuditEvent` enum, `AuditService`, `@Aspect` interceptor,
events `LOGIN_SUCCESS/FAILURE/LOGOUT/TOKEN_REFRESH/FILE_UPLOAD/ADMIN_ACCESS` ;
`GET /api/v1/files/:id/download-url` presigned URL courte durée via `MinioClient` ;
`${CORS_ALLOWED_ORIGINS}` env var injectable. Critère de succès : B1 fermé + C9 complet + CORS injectable.
Après SB7 : relancer la revue V1 §30 pour décision `VALIDE_V1`.
