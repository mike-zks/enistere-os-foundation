...
> **Source de vérité = le repository**, résumé par `docs/project-status/`. Vérifié le 2026-07-15.
...
- **Vides** : aucun. **`ai-core`** → **`IMPLEMENTATION_AVANCEE`** (AI Core 11, 2026-07-17) : §18 **7/7 satisfaits** ; B1 prompt runner fermé ; B2 retrieval citations fermé ; B3 runbook fermé. Livrables existants : `CORE_SPECIFICATION.md` + `README.md` + `AI_CORE_USAGE_RUNBOOK.md` + registre local `prompt-registry.json` + validateur Node pur (`src/prompt-registry/*`, `scripts/validate-prompt-registry.mjs`, tests) + redaction layer pure (`src/redaction/*`, tests) + context builder allow-list (`src/context/*`, tests) + provider seam/fake provider (`src/provider/*`, tests) + evaluation harness local (`src/evaluation/*`, tests) + décision Retrieval/RAG V1 (`AI_CORE7_RETRIEVAL_RAG_DECISION.md`) + schema local de rapport d'execution (`src/reports/*`, tests) + runner de prompt gouverné fake-provider (`src/runner/*`, tests) + citations retrieval (`src/retrieval/*`, tests). `VALIDE_V1` n'est pas déclaré automatiquement : prochaine action recommandée = **AI Core V1 Final Readiness Decision**. Aucun provider réel, SDK IA, LLM judge, RAG runtime, base vectorielle, endpoint, dépendance, secret ou appel réseau. **`web-angular`** → **`VALIDE_V1`** (Angular 10 + Quality Web Angular CI, 2026-07-16) : B1 RefreshInterceptor/login API seam fermé ; B2 PermissionService/PermissionDirective fermé ; readiness finale **14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT**. Réserve non-bloquante restante : R1 CDK a11y FocusTrap/ARIA HTML. R2 CI gate fermée par `web-angular-ci.yml` + scope `quality-gates web-angular`. **267/267 tests ✅**.
...
automatique ou stockage de traces. Prochaine action : **AI Core V1 Final Readiness Decision**.
