...
- **Vides** : `ai-core`. **`web-angular`** → **`STARTER_INITIALISE`** (Angular 2, 2026-07-15) : starter Angular 22.0.6 + Material/CDK 22.0.4 + thème M3 Enistere (`mat.define-theme()`, tokens `--enistere-*` → `--mat-sys-*`, dark mode `[data-theme='dark']`) ; 8/8 tests ✅ (ChromeHeadless 150.0.0.0) ; build SUCCESS 340 KB. Contrainte Node : Angular CLI 22.x requiert `^22.22.3 || ^24.15.0` ; env = Node 24.14.0 → build/tests via `./node_modules/.bin/architect`. ADR-035 fondateur. Prochaine action : **Web Core Angular 3 — Auth flow + routing protégé**.
...
**Web Core Angular 2 — Starter minimal Angular** (`cores/web-angular/`, `docs/project-status/`) (2026-07-15) : starter Angular 22.0.6 créé. `web-angular` → **`STARTER_INITIALISE`**. Livrables : `package.json` (@angular/core 22.0.6, @angular/material/cdk 22.0.4, typescript 6.0.3, @angular/cli 22.0.7 + @angular/build 22.0.7, override `vite` 7.3.6 pour audit 0 ; `engines.node: >=24.15.0 || >=22.22.3`), `angular.json` (builder `@angular/build:application`, esbuild), `tsconfig.json` (strict + `strictTemplates` + ES2022 + `useDefineForClassFields: false`), `karma.conf.js` (ChromeHeadlessNoSandbox — `--no-sandbox --disable-gpu --disable-dev-shm-usage`, `singleRun`), `src/main.ts` (`bootstrapApplication(AppComponent, appConfig)`), `src/styles.scss` (thème Material 3 : `mat.define-theme()` + `mat.$azure-palette` + `mat.$cyan-palette`, `@include mat.all-component-themes()`, tokens Enistere `--enistere-*` → `--mat-sys-*`, dark mode `[data-theme='dark']`), `src/app/app.config.ts` (`provideRouter(routes, withComponentInputBinding())`, `provideHttpClient(withFetch())`, `provideAnimationsAsync()`), `src/app/app.routes.ts` (lazy `HomeComponent`, wildcard `''`), `src/app/app.component.ts` (standalone, `RouterOutlet`), `src/app/pages/home/home.component.ts` (page shell, `.status-value = 'STARTER_INITIALISE'`). Tests : **8/8** ✅ (3 AppComponent + 5 HomeComponent, ChromeHeadless 150.0.0.0). Build : `architect web-angular:build` SUCCESS (340 KB initial, zéro erreur TypeScript). Contrainte Node : Angular 22.x requiert `^22.22.3 || ^24.15.0 || >=26.0.0` ; les preuves locales utilisent le runtime compatible et `architect` pour un code de sortie déterministe. Aucun script `lint` n'est exposé tant qu'Angular ESLint n'est pas installé/configuré. Mises à jour documentaires : DECISIONS_REGISTER.md + FOUNDATION_CURRENT_STATE.md + NEXT_ACTIONS.md + IMPLEMENTATION_MATRIX.md + SESSION_HANDOFF.md + CHANGELOG.md. `quality-gates docs` 2/2 ✅ · `git diff --check` ✅. **Prochaine action : Web Core Angular 3 — Auth flow + routing protégé**.
