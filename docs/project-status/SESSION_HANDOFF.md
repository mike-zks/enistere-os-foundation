...
- **Vides** : `ai-core`. **`web-angular`** → **`AUTH_ROUTING_READY`** (Angular 3, 2026-07-16) : starter Angular 22.0.6 + Material/CDK 22.0.4 + thème M3 Enistere (`mat.define-theme()`, tokens `--enistere-*` → `--mat-sys-*`, dark mode `[data-theme='dark']`) ; auth shell Signals (`AuthService` read-only public state, access token mémoire privé), guards `authGuard`/`guestGuard`, `sanitizeReturnUrl` anti open-redirect, `AuthInterceptor` Bearer mémoire, routes `/login` et `/dashboard`, pages shells accessibles. ADR-035 fondateur. Prochaine action : **Web Core Angular 4 — Client HTTP + server state**.
