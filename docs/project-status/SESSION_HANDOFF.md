...
- **Vides** : `ai-core`. **`web-angular`** → **`HTTP_SERVER_STATE_READY`** (Angular 4, 2026-07-16) : starter Angular 22.0.6 + Material/CDK 22.0.4 + thème M3 Enistere ; auth shell Signals Angular 3 conservé ; couche HTTP Angular 4 livrée (`APP_BASE_URL`, `AppApiError`/`mapHttpError`, `ErrorInterceptor`, `LogInterceptor`, `RequestState<T>`/`createRequestState`) avec ordre d'intercepteurs `auth → log → error`, 401 surfacé sans refresh silencieux, logs méthode+path+status+duration uniquement (aucun body/header/token/query/URL signée). ADR-035 fondateur. Prochaine action : **Web Core Angular 5 — Reactive Forms + Angular Material**.
