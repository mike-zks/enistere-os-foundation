...
- **Vides** : `ai-core`. **`web-angular`** → **`IMPLEMENTATION_AVANCEE`** (Angular 9, 2026-07-16) : B1 RefreshInterceptor/login API seam fermé ; readiness mise à jour **13/15 §29 SATISFAIT + 1/15 PARTIEL + 1/15 NON SATISFAIT**. `VALIDE_V1` différé par B2 PermissionService/PermissionDirective. Réserves non-bloquantes : R1 CDK a11y FocusTrap/ARIA HTML, R2 CI gate/§32. **248/248 tests ✅**. Rapports : `WEB_ANGULAR_V1_READINESS_REVIEW.md`, `WEB_ANGULAR9_REFRESH_INTERCEPTOR_REPORT.md`. Prochaine action : **Web Core Angular 10 — PermissionService + PermissionDirective**.
...
**Web Core Angular 9 — RefreshInterceptor + login API seam** (`cores/web-angular/`, `docs/project-status/`) (2026-07-16) : `web-angular` reste **`IMPLEMENTATION_AVANCEE`**, mais B1 est fermé. Livrables : `AuthApi` seam + `PlaceholderAuthApi`, `AuthService.login()` Observable, access token mémoire privé, purge token sur erreur login, `refreshSession()` coalescé, `RefreshInterceptor` 401→refresh→retry unique→logout/purge si échec, endpoints auth exclus, retry `FormData` sans `Content-Type` forcé. Rapport `WEB_ANGULAR9_REFRESH_INTERCEPTOR_REPORT.md`. Tests : **248/248** ✅. Readiness : **13/15 §29 SATISFAIT + 1/15 PARTIEL + 1/15 NON SATISFAIT**. Prochaine action : **Web Core Angular 10 — PermissionService + PermissionDirective**.
