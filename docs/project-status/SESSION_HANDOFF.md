...
- **Vides** : `ai-core`. **`web-angular`** → **`SPECIFICATION_DOCUMENTAIRE`** (Angular 1, 2026-07-15) : `CORE_SPECIFICATION.md` (32 §) + `README.md` livrés — ADR-035 fondateur (Angular Material CDK + M3 + tokens Enistere) ; architecture standalone, routing, guards, intercepteurs, auth/session, Reactive Forms, thème `mat.define-theme()`, composants maison, RBAC, tests attendus, 15 critères V1 (§29), 13 décisions pendantes (§32). Aucun starter Angular, aucun code TypeScript, aucune dépendance npm. Prochaine action : **Web Core Angular 2 — Starter minimal Angular**.
