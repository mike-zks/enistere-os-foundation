...
- **`IMPLEMENTATION_PARTIELLE` / `PERSISTENCE_RBAC_READY`** : `api-spring` (Spring Boot 3, 2026-07-15 — PostgreSQL + Spring Data JPA + Flyway 11 + Argon2id (ADR-039) ; entités JPA User/Role/Permission/RefreshToken (hash SHA-256) ; migration V1 6 tables + 5 index ; `AuthService` (login Argon2 verify, refresh rotation, logout, me DTO) ; `EnistereUserDetailsService` (DB-backed) ; JWT claims `userId`+`permissions[]` stateless ; `@PreAuthorize("hasAuthority(...)")` RBAC ; `AdminController` GET /api/v1/admin/ping ; Testcontainers singleton `@DynamicPropertySource` ; `./mvnw verify` **43/43 ✅ BUILD SUCCESS**. Spring Boot 2 base conservée : `pom.xml` Spring Boot 4.1.0 + JJWT 0.12.6 + Java 21 + Maven Wrapper 3.9.12 + `JwtAuthenticationFilter` + CORS). ADR-041 Maven. Prochain : **Spring Boot 4 — OpenAPI + Upload MinIO**.
