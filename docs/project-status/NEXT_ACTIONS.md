...
> ✅ **Mobile Core React Native 1→18 : RÉALISÉS** (`mobile-react-native` → **`BIOMETRIC_GATE_READY`**). RN 4/4B/5 :
...
> aucun secret/biométrie/résultat stocké.** **262 tests `node --test`**. Vérifs : **typecheck + lint + test 262/262 +
> expo-doctor 19/19 + git diff --check verts** (**RN 18 n'ajoute aucune dépendance**). **Aucune logique métier.**
> *(Garde CI `npm ls zustand` au root inchangée — mobile autonome, hors scope.)*
>
> **Prochaine action UNIQUE : Mobile Core React Native 19 — crash / error-reporting primitives génériques (seam, sans SDK
> réel — ADR-019)** : `CrashReporterAdapter` (seam futur Sentry/Crashlytics) + modèle d'événement d'erreur **borné et
> rédigé** (réutilise la **redaction RN 8** : ni token/PII/contenu utilisateur, ni stack brute non filtrée) + service
> (`captureError`/`captureMessage`/`setContext` sûr) + placeholder mémoire, **mappé purement** et **testable**, **sans
> Sentry/Crashlytics/Bugsnag réel, sans réseau, sans persistance, sans donnée sensible**, **non-intrusif** (ne casse jamais
> le flux). **Un seul core**, **sans logique métier**. Différés au-delà : **adaptateurs natifs réels** (biométrie, crash
> SDK, remote-config), **offline sync réelle** (ADR-029).
>
...
19. **Mobile Core React Native 19 — crash / error-reporting primitives génériques (seam, sans SDK réel — ADR-019)** ✦ **prochaine mission** — `CrashReporterAdapter` (seam futur Sentry/Crashlytics) + événement d'erreur **borné et rédigé** (redaction RN 8) + service (`captureError`/`captureMessage`/`setContext`) + placeholder mémoire, mappé purement et testable, **sans Sentry/Crashlytics réel/réseau/persistance/donnée sensible**, **non-intrusif**.
20. **UI Kit 4** — primitives interactives (Dialog/Select/Toast) — débloque Mobile/Web riches.
21. **Cloud Core 10 — préparation serveur staging sécurisé** — **reporté** (dépend d'un serveur réel + HTTPS/DNS/pare-feu ; Cloud en **pause contrôlée**).
22. **Web Core Files 2** — upload sécurisé côté Web (multipart, finalisation, états).
...
| **Mobile Core React Native 19 — crash / error-reporting primitives génériques (seam, sans SDK réel — ADR-019)** | **PROCHAINE MISSION** — `CrashReporterAdapter` (seam futur Sentry/Crashlytics) + événement d'erreur **borné et rédigé** (réutilise la **redaction RN 8** : ni token/PII/contenu utilisateur, ni stack brute non filtrée) + service (`captureError`/`captureMessage`/`setContext` sûr) + **placeholder** mémoire, **mappé purement** et **testable**, **sans Sentry/Crashlytics/Bugsnag réel/réseau/persistance/donnée sensible**, **non-intrusif** (ne casse jamais le flux) |
