...
> ✅ **Mobile Core React Native 1→17 : RÉALISÉS** (`mobile-react-native` → **`FEATURE_FLAGS_READY`**). RN 4/4B/5 :
...
> valeur**). **Aucun SDK remote-config réel/réseau/persistance/user targeting réel/secret/donnée sensible.** **244 tests
> `node --test`**. Vérifs : **typecheck + lint + test 244/244 + expo-doctor 19/19 + git diff --check verts** (**RN 17
> n'ajoute aucune dépendance**). **Aucune logique métier.**
> *(Garde CI `npm ls zustand` au root inchangée — mobile autonome, hors scope.)*
>
> **Prochaine action UNIQUE : Mobile Core React Native 17 — feature flags / config primitives génériques** : `FlagAdapter`
> (seam future source de config) + flags **typés** (boolean/string/number) avec **défauts sûrs** + bornes + service
> (`getFlag`/`getAll`/`subscribe`) + placeholder, **mappé purement** et **testable**, **sans SDK remote-config réel, sans
> réseau, sans persistance, sans donnée sensible** (séparé des `flags` UI Zustand RN 6). **Un seul core**, **sans logique
> métier**. Différés au-delà : **adaptateurs natifs réels**, **SDK remote-config réel**, **offline sync réelle** (ADR-029).
>
...
18. **Mobile Core React Native 18 — gate biométrique local primitives génériques (ADR-015 §20)** ✦ **prochaine mission** — `BiometricAdapter` (seam future `LocalAuthentication`/Keychain) + disponibilité/résultat normalisés + service (`isAvailable`/`authenticate`) + placeholder, mappé purement et testable, **sans `LocalAuthentication` réel, sans secret/biométrie stockée**, la biométrie ne remplaçant **jamais** l'auth serveur (gate local d'UX).
19. **UI Kit 4** — primitives interactives (Dialog/Select/Toast) — débloque Mobile/Web riches.
20. **Cloud Core 10 — préparation serveur staging sécurisé** — **reporté** (dépend d'un serveur réel + HTTPS/DNS/pare-feu ; Cloud en **pause contrôlée**).
21. **Web Core Files 2** — upload sécurisé côté Web (multipart, finalisation, états).
...
| **Mobile Core React Native 18 — gate biométrique local primitives génériques (ADR-015 §20)** | **PROCHAINE MISSION** — `BiometricAdapter` (seam future `LocalAuthentication`/Keychain) + disponibilité/résultat **normalisés** + service (`isAvailable`/`authenticate`) + placeholder, **mappé purement** et **testable**, **sans `LocalAuthentication` réel, sans secret/biométrie stockée** ; la biométrie **ne remplace jamais** l'auth serveur (gate local d'UX uniquement) |
