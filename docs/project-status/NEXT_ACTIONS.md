...
> ✅ **Mobile Core React Native 1→16 : RÉALISÉS** (`mobile-react-native` → **`NETWORK_STATUS_READY`**). RN 4/4B/5 :
...
> persistance/donnée sensible.** **227 tests `node --test`**. Vérifs : **typecheck + lint + test 227/227 + expo-doctor
> 19/19 + git diff --check verts** (**RN 16 n'ajoute aucune dépendance**). **Aucune logique métier.**
...
17. **Mobile Core React Native 17 — feature flags / config primitives génériques** ✦ **prochaine mission** — `FlagAdapter` (seam future config) + flags typés (boolean/string/number) avec défauts sûrs + bornes + service (`getFlag`/`getAll`/`subscribe`) + placeholder, mappé purement et testable, sans SDK remote-config réel/réseau/persistance.
18. **UI Kit 4** — primitives interactives (Dialog/Select/Toast) — débloque Mobile/Web riches.
19. **Cloud Core 10 — préparation serveur staging sécurisé** — **reporté** (dépend d'un serveur réel + HTTPS/DNS/pare-feu ; Cloud en **pause contrôlée**).
20. **Web Core Files 2** — upload sécurisé côté Web (multipart, finalisation, états).
...
| **Mobile Core React Native 17 — feature flags / config primitives génériques** | **PROCHAINE MISSION** — `FlagAdapter` (seam future source de config/remote-config) + flags **typés** (boolean/string/number) avec **défauts sûrs** + bornes + service (`getFlag`/`getAll`/`subscribe`) + placeholder, **mappé purement** et **testable**, **sans SDK remote-config réel/réseau/persistance/donnée sensible** (séparé des `flags` UI Zustand RN 6) |
