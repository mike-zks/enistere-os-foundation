...
> **4 / 15 PARTIEL** avec réserves formellement acceptées : R1 RefreshInterceptor/login() placeholder (seam présent — §29.3+§29.5), R2 PermissionDirective (display-UX déléguée projet dérivé — §29.13), R3 CDK a11y FocusTrap/LiveAnnouncer (ARIA HTML équivalent — §29.9), R4 CI Angular gate (non §29, reporté §32).
...
> `home.component.html` badge `TEST_SMOKE_READY` → `VALIDE_V1`. Tests : **224/224 ✅** (aucun test ajouté, aucune dépendance).
...
| Web Core Angular | ✓ | **✓ (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15)** | **ADR-035 (Validé, 2026-07-15)** | **✓ (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 `mat-form-field`+`matInput`+`mat-error`+`getFieldError()` + Angular 6 Foundation state components + Angular 7 Upload fichiers + Angular 8 Tests+smoke)** | **✓ (Angular 3→8 — HTTP, intercepteurs, server-state RxJS, Angular Material, Foundation states, FileCategory/AppFile/UploadService/UploadFormComponent, CDK harnesses, RouterTestingHarness)** | **✓ (224/224 ; Angular 3→7 + 16 nouveaux tests Angular 8 : RouterTestingHarness navigation/guards, MatFormFieldHarness/MatInputHarness login, MatSelectHarness/MatButtonHarness/MatProgressSpinnerHarness upload+loading)** | **✓ (`WEB_ANGULAR_V1_READINESS_REVIEW.md` — 11/15 §29 SATISFAIT + 4/15 PARTIEL ; réserves R1–R4 formellement acceptées non-bloquantes)** | **VALIDE_V1** | `architect web-angular:build` SUCCESS · `architect web-angular:test` 224/224 ✅ · `npm audit` 0 vuln · `quality-gates docs` 2/2 ✅ | — (V1 déclaré) |
