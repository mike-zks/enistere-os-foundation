...
> **Mise à jour UI Kit V1 Readiness Review (2026-07-11)** : Le UI Kit passe de **`IMPLEMENTATION_PARTIELLE`** à **`IMPLEMENTATION_AVANCEE`** après revue officielle (rapport `UI_KIT_V1_READINESS_REVIEW.md`). Score : **3/4 critères §12.4** + **8/9 critères §59** ; gap bloquant VALIDE_V1 = composants React Native de base (différés ADR-010). Prochaine mission : **Mobile RN35**. Mobile Core React Native reste **`STARTER_EXPO_DOCTOR_GREEN`** (RN34 + smoke Android RN34B passé ; RN31 iOS bloqué Linux). UI Kit : **19 primitives Web React** (UI Kit 4 : Dialog/Select/Toast + UI Kit 5 : Badge/Divider/Skeleton + UI Kit 6 : LoadingState/EmptyState/ErrorState/SuccessState) — **181 tests** ; `Dialog` marqué `'use client'` pour Next Server Components.
...
| UI Kit (`@enistere/ui-kit`) | ✓ | ✓ | ✓ (008,009,010) | **✓** | **✓ (tokens + 19 primitives Web : Button/Input/Label/Text/Spinner/VisuallyHidden + Alert/Card/FormField + Dialog/Select/Toast + Badge/Divider/Skeleton + LoadingState/EmptyState/ErrorState/SuccessState)** | **✓ (181, a11y/jest-axe, React 19)** | **✓ (UI Kit V1 Readiness Review 2026-07-11 : 3/4 critères §12.4 ; 8/9 critères §59 ; rapport `UI_KIT_V1_READINESS_REVIEW.md`)** | **IMPLEMENTATION_AVANCEE** | aligné **React 19** (0 régression, v0.1.1) + **réellement consommé par le Web Core VALIDE_V1** ; Dialog marqué `'use client'` pour compatibilité Next Server Components ; pack:check OK ; **gap bloquant VALIDE_V1 : composants React Native de base (différés ADR-010)** | **Mobile RN35** — aligner le starter mobile avec les états UI / UI Kit (ThemeProvider basé tokens + primitives d'état RN) |
