...
| Cloud Core | ✓ | ✓ | ✓ (013,014,007…) | — | **cadrage (CC1) + CI runtime API (CC2) + CI E2E navigateur (CC3)** : `api-runtime-ci.yml` (PG+MinIO, migrations, unit+e2e, openapi:check) **+ `web-e2e-ci.yml`** (stack réelle + Playwright/Chromium : Health/Auth/Files) | **e2e API + E2E navigateur en CI** (niveaux 2–3) | — | **IMPLEMENTATION_PARTIELLE** | **quatre workflows CI** niveaux 1–4 partiel (`ci`/`api-runtime`/`web-e2e`/**`registry`** — Registry CI verte sur `main`, **images GHCR publiques**) **+ gouvernance CC4 + staging manuel CC6 + dry-run contrôlé CC7** (`DRY_RUN_EXECUTE` : **défaut bloquant — image API ne démarre pas**, image Web OK) ; **aucune exécution de déploiement** | **Cloud Core 8 — corriger l'image runtime API (moteur Prisma)** puis re-dry-run + stratégie migrations ; **puis** exécution réelle staging |
...
| Registry images | ADR-014 Validé | **`registry-ci.yml`** + Dockerfiles API/Web (build + push GHCR sur `main`, tags immuables, non-root, sans secret/PAT) | — (validé localement : `docker build` API+Web OK + smoke) | **PARTIELLEMENT_IMPLEMENTE** | déploiement, scan/signature d'image, semver/release |
| Conteneurisation (Docker) | ADR-014 | **Dockerfiles API/Web** (multi-stage, non-root ; Web standalone) + `.dockerignore` ; **compose staging exemple** (CC6) | ✓ build/config ; ⚠️ **dry-run CC7 : image Web boote, image API crash-loop** (moteur Prisma OpenSSL 1.1.x vs runtime bookworm 3.0.x) | **PARTIELLEMENT_IMPLEMENTE** (**défaut image API**) | **corriger l'image API (CC8)** ; puis compose de prod, Traefik |
| Déploiement staging | ADR-013 | runbooks + compose/`.env` exemples (CC6) + **dry-run contrôlé exécuté** (CC7 : `compose config`/`pull` OK, postgres/minio/bucket, image Web HTTP 200) | ⚠️ **dry-run réel** : **image API ne démarre pas** → `/health/ready` jamais vert ; migrate-from-source non exercé (egress bloqué) | **DRY_RUN_EXECUTE** (**défaut bloquant**) | **CC8 — corriger l'image API** puis re-dry-run, puis exécution réelle (serveur + `S3_ENDPOINT` public, Option A) |
