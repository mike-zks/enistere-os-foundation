...
| Mobile Core React Native | ✓ | ✓ | ✓ (003,004,008,010,011,012,015,016…) | **✓ (Expo SDK 55 + Expo Router)** | **✓ (RN 1 socle + RN 2 hardening + RN 3 forms/offline + RN 4 client officiel)** : AuthEngine agnostique (refresh coalescé, expiration) **préservé** ; SessionStore SecureStore + validation, access token mémoire ADR-015 ; **client OFFICIEL `@enistere/api-client-fetch` + `@enistere/api-contracts` intégré** (ADR-016 ; liés `file:` + `metro.config.js`, **core autonome — root non touché**) via `MobileAuthSessionAdapter` (injection Bearer, aucun token stocké) + `EnistereAuthApi` (`/auth/login`+`/auth/refresh` typés) ; `401`→refresh→retry possédé par l'AuthEngine (`enableRefresh:false`) ADR-011 ; erreurs `ApiClientError` ; TanStack Query ADR-012 ; ThemeProvider ADR-008/010 ; **forms RHF + Zod** + **offline préparatoire** | **✓ (41 tests `node --test` : auth-engine/session-store/validation/form-errors/offline-queue/network-state/token-mapping ; + bundle `expo export` ios)** | — | **API_CLIENT_INTEGRATED** | typecheck + lint + **test 41/41** + **expo-doctor 19/19** + **bundle Metro** verts (local) | **Mobile Core RN 5 — server-state data layer** (hooks TanStack Query au-dessus du client ; Zustand/upload/offline-sync différés) |
