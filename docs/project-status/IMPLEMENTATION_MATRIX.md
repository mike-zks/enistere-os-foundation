...
| Mobile Core React Native | ✓ | ✓ | ✓ (003,004,008,010,011,012,015,016…) | **✓ (Expo SDK 55 + Expo Router)** | **✓ (RN 1 socle + RN 2 hardening + RN 3 forms/offline)** : AuthEngine agnostique (refresh coalescé, expiration) ; SessionStore SecureStore + validation, access token mémoire ADR-015 ; API client `401`→refresh→retry ADR-011 ; gardes expired/refreshing ; seam `@enistere/api-client-fetch` ADR-016 ; TanStack Query ADR-012 ; ThemeProvider ADR-008/010 ; états standards ; **formulaires RHF + Zod (FormField/FormLabel/FormError/TextInputField ; validation UX `validateWith`+mapping erreurs, ADR-003 §18 — backend autoritatif)** ; **offline préparatoire (état réseau abstrait + queue mémoire FIFO ; sans persistance/rejeu/NetInfo/donnée sensible, ADR-015 §19)** | **✓ (44 tests `node --test` : auth-engine/session-store/api-client/validation/form-errors/offline-queue/network-state)** | — | **FORMS_OFFLINE_PRIMITIVES_READY** | typecheck + lint + **test 44/44** + **expo-doctor 19/19** verts (local) | **Mobile Core RN 4 — intégration réelle `@enistere/api-client-fetch`** (workspace racine + Metro ; Zustand/upload/offline-sync différés) |
...
| C6 | `cores/{cloud,mobile-react-native}` ont une spéc | Starter | `cloud` = PARTIEL/PAUSE ; **`mobile-react-native` `FORMS_OFFLINE_PRIMITIVES_READY`** (Expo SDK 55, socle + auth/session durci + forms/validation + offline préparatoire, 44 tests) ; ui-kit/web-nextjs `STARTER_INITIALISE` | Confusion spéc↔implémentation | Statut explicite (cette matrice) ; mobile = socle durci ≠ V1 complète | IMPORTANTE |
