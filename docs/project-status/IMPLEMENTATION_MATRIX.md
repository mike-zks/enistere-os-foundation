...
| C6 | `cores/{cloud,mobile-react-native}` ont une spéc | Starter | `cloud` = PARTIEL/PAUSE ; **`mobile-react-native` `NOTIFICATIONS_READY`** (Expo SDK 55, socle + auth/session + forms/validation + offline préparatoire + **client officiel + server-state + état local UI + purge logout + primitives upload multipart + logger/redaction + permissions runtime + notifications locales**, 122 tests + bundle Metro) ; ui-kit/web-nextjs `STARTER_INITIALISE` | Confusion spéc↔implémentation | Statut explicite (cette matrice) ; mobile = socle durci ≠ V1 complète | IMPORTANTE |
