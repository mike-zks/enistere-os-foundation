...
| `@enistere/api-client-fetch` | ✓ | n/a | ✓ (011,012,016) | ✓ | ✓ | ✓ (29 + live 16/16) | ✓ (proof) | **IMPLEMENTATION_AVANCEE** (local) | live 16/16 ; **instancié (public) dans le Web Core** | intégration authentifiée (BFF) |
| Cloud Core | ✓ | ✓ | ✓ (013,014,007…) | — | — | — | — | **SPECIFICATION_DOCUMENTAIRE** | — | starter (après CI/registry) |
| Web Core Next.js | ✓ | ✓ | ✓ (004,005,009,011,012,016…) | **✓** | **✓ (App Router + UI Kit + API publique Health + TanStack Query, SSR/hydratation + fondations BFF Auth serveur)** | **✓ (112, a11y + sonde HTTP + preuve API réelle)** | — | **IMPLEMENTATION_PARTIELLE** | build/lint/typecheck/112 tests verts + preuve API réelle ; fondations Auth (client serveur authentifiable, cookies HttpOnly, session adapter) **sans route ni CSRF** | Web Auth 2 (login/refresh/logout via Route Handlers BFF + CSRF) |
...
| C2 | Packages dits « officiels » | Import dans les cores | **Intégré (public)** : UI Kit **consommé** + `api-contracts`/`api-client-fetch` **instanciés** par le Web Core pour les endpoints **publics** (Health), preuve API réelle. Reste : usage **authentifié** (BFF) | Lecture « intégré » désormais vraie pour le public | Intégration authentifiée (Auth/BFF) | MOYENNE |
