...
> **11 / 15 SATISFAIT** (§29.1 build, §29.2 router+guards, §29.4 tokens mémoire, §29.6 Reactive Forms, §29.7 Material 3 thème, §29.8 Foundation components, §29.10 tests auth, §29.11 tests composants 224/224, §29.12 HTTP typé, §29.14 Chrome, §29.15 ng build production).
> **3 / 15 PARTIEL** (§29.3/§29.5 refresh/login API seam, §29.9 CDK a11y) + **1 / 15 NON SATISFAIT** (§29.13 PermissionService/PermissionDirective absent).
> Blockers V1 : **B1 RefreshInterceptor/login API seam**, **B2 PermissionService + PermissionDirective**. Réserves non-bloquantes : R1 CDK a11y FocusTrap/LiveAnnouncer (ARIA HTML équivalent), R2 CI Angular gate (non §29, reporté §32).
...
> `home.component.html` badge `TEST_SMOKE_READY` → `IMPLEMENTATION_AVANCEE`. Tests : **224/224 ✅** (aucun test ajouté, aucune dépendance).
> `web-angular` : **`TEST_SMOKE_READY` → `IMPLEMENTATION_AVANCEE`**. `VALIDE_V1` différé jusqu'à fermeture B1/B2.
>
...
| Web Core Angular | ✓ | **✓ (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15)** | **ADR-035 (Validé, 2026-07-15)** | **✓ (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 `mat-form-field`+`matInput`+`mat-error`+`getFieldError()` + Angular 6 Foundation state components + Angular 7 Upload fichiers + Angular 8 Tests+smoke)** | **✓ (Angular 3→8 — HTTP, intercepteurs, server-state RxJS, Angular Material, Foundation states, FileCategory/AppFile/UploadService/UploadFormComponent, CDK harnesses, RouterTestingHarness)** | **✓ (224/224 ; Angular 3→7 + 16 nouveaux tests Angular 8 : RouterTestingHarness navigation/guards, MatFormFieldHarness/MatInputHarness login, MatSelectHarness/MatButtonHarness/MatProgressSpinnerHarness upload+loading)** | **✓ (`WEB_ANGULAR_V1_READINESS_REVIEW.md` — 11/15 §29 SATISFAIT + 3/15 PARTIEL + 1/15 NON SATISFAIT ; B1/B2 bloquants V1)** | **IMPLEMENTATION_AVANCEE** | `architect web-angular:build` SUCCESS · `architect web-angular:test` 224/224 ✅ · `npm audit` 0 vuln · `quality-gates docs` 2/2 ✅ | Angular 9 RefreshInterceptor/login API seam ; Angular 10 PermissionService/PermissionDirective |
