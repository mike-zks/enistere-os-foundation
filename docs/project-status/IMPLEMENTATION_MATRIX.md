...
| Mobile Core React Native | ✓ | ✓ | ✓ (003,004,008,010,011,012,015,016…) | **✓ (Expo SDK 55 + Expo Router)** | **✓ (RN 1 socle + RN 2 hardening + RN 3 forms/offline + RN 4 client officiel)** : AuthEngine agnostique (refresh coalescé, expiration) **préservé** ; SessionStore SecureStore + validation, access token mémoire ADR-015 ; **client OFFICIEL `@enistere/api-client-fetch` + `@enistere/api-contracts` intégré** (ADR-016 ; liés `file:` + `metro.config.js`, **core autonome — root non touché**) via `MobileAuthSessionAdapter` (injection Bearer, aucun token stocké) + `EnistereAuthApi` (`/auth/login`+`/auth/refresh` typés) ; **`401`→`refreshSession` coalescé→1 retry→purge possédé par l'AuthEngine** (`enableRefresh:false` + pont `withAuthRetry`/`authedRequest`, **RN 4B**) ADR-011 ; erreurs `ApiClientError` ; ThemeProvider ADR-008/010 ; **forms RHF + Zod** + **offline préparatoire** **+ RN 5 — couche server-state TanStack Query générique** (ADR-012 : `createQueryKeys` clés stables typées, `useAuthedQuery`/`useAuthedMutation` **via `authedRequest`**, `toQueryError` sans donnée sensible, `invalidateScope`/`purgeServerState` au logout ; **401 jamais retenté, mutations sans retry, pas de persistance, aucun endpoint métier**) | **✓ (59 tests `node --test` : … + with-auth-retry + query-keys + query-errors ; + bundle `expo export` ios)** | — | **SERVER_STATE_READY** | typecheck + lint + **test 59/59** + **expo-doctor 19/19** + **bundle Metro** verts (local) | **Mobile Core RN 6 — état local (Zustand) + câblage `signOut → purgeServerState`** (upload/notifications/offline-sync différés) |
...
| C6 | `cores/{cloud,mobile-react-native}` ont une spéc | Starter | `cloud` = PARTIEL/PAUSE ; **`mobile-react-native` `SERVER_STATE_READY`** (Expo SDK 55, socle + auth/session + forms/validation + offline préparatoire + **client officiel intégré + couche server-state**, 59 tests + bundle Metro) ; ui-kit/web-nextjs `STARTER_INITIALISE` | Confusion spéc↔implémentation | Statut explicite (cette matrice) ; mobile = socle durci ≠ V1 complète | IMPORTANTE |
