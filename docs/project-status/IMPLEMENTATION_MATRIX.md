...
| Web Core Next.js | ✓ | ✓ | ✓ (004,005,006,009,011,012,016…) | **✓** | **✓ (App Router + UI Kit + API publique Health + TanStack Query + BFF Auth + me/authorization + session state + premier layout protégé : résolution Auth serveur read-only Option C + hydratation + page /protected)** | **✓ (230, a11y + sonde HTTP + preuve API réelle Auth + session + protégé 26/26)** | **✓ (revue de gouvernance 2026-06-10 — `WEB_CORE_GOVERNANCE_REVIEW.md`)** | **IMPLEMENTATION_PARTIELLE** | build/lint/typecheck/**230 tests** verts + couverture + preuve API réelle (Auth/session : login→/me→/authorization→logout→/me 401, read-only sans refresh, droits sans nouveau JWT ; **protégé** : anonyme→redirection serveur sans donnée privée, authentifié→200+profil hydraté sans token, API arrêtée→« indisponible » ≠ anonyme ; bundle sans secret) | **Web Auth 5** — page de connexion & navigation Auth (remplace `/?auth=required`) ; CI minimale recommandée |
