...
| Web Core Next.js | ✓ | ✓ | ✓ (004,005,006,009,011,012,016…) | **✓** | **✓ (App Router + UI Kit + API publique Health + TanStack Query + BFF Auth + me/authorization + session state + layout protégé serveur read-only Option C + hydratation + page /protected + page de connexion /login : formulaire, login BFF, returnTo interne assaini, navigation replace/refresh)** | **✓ (263, a11y + sonde HTTP + preuve API réelle Auth/session + protégé 26/26 + login 22/22)** | **✓ (revue de gouvernance 2026-06-10 — `WEB_CORE_GOVERNANCE_REVIEW.md`)** | **IMPLEMENTATION_PARTIELLE** | build/lint/typecheck/**263 tests** verts + couverture + preuves API réelles (Auth/session ; protégé ; **login** : anonyme→redirection /login, login BFF sans token, déjà-authentifié→redirection hors login, **returnTo externe→/protected (aucun open redirect)**, 401 sans énumération, 403 CSRF, sans fuite token/mot de passe) | **Revue globale Auth Web (1→5)** — audit transversal + décision stabilité V1 ; CI minimale recommandée |
