...
| AI Core | ✓ | ✓ (`CORE_SPECIFICATION.md`, `README.md` — AI Core 1, 2026-07-16) | — | — | ✓ (`prompt-registry.json` + validateur Node pur + redaction layer + context builder + provider fake + evaluation harness — AI Core 2→6) | ✓ (tests Node + validation registry + redaction + context + provider + evaluation) | — | **IMPLEMENTATION_PARTIELLE** | `validate-prompt-registry` + `node --test` + `quality-gates docs` | AI Core 7 — Retrieval/RAG design decision |
