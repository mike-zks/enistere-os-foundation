...
> **`STARTER_UI_KIT_ALIGNED`** à **`IMPLEMENTATION_AVANCEE`**. Critères roadmap §9.4 :
> **7/8 satisfaits**, upload runtime mobile encore partiel. `VALIDE_V1` différé par trois réserves :
...
| C6 | `cores/{cloud,mobile-react-native}` ont une spéc | Starter | `cloud` = **VALIDE_V1** ; **`mobile-react-native` `IMPLEMENTATION_AVANCEE`** (Expo SDK 55, primitives RN 1→35, Settings, runtime Android, iOS bloqué par Linux sans `xcrun`, formulaire, thème, doctor 19/19, alignement UI Kit) ; `VALIDE_V1` mobile différé par upload runtime/iOS/store natif non sensible | Confusion spéc↔implémentation | Statut explicite (cette matrice) ; mobile = socle avancé ≠ V1 complète | IMPORTANTE |
