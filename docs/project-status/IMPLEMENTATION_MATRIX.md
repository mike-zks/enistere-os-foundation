...
> Zéro nouvelle dépendance. `home.component.html` badge `TEST_SMOKE_READY`. Rapport `WEB_ANGULAR8_TESTS_SMOKE_REPORT.md`. Tests : **224/224 ✅**.
> `web-angular` : **`UPLOAD_READY` → `TEST_SMOKE_READY`**. Prochaine action : Web Core Angular V1 Readiness Review.
>
...
| Web Core Angular | ✓ | **✓ (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15)** | **ADR-035 (Validé, 2026-07-15)** | **✓ (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 `mat-form-field`+`matInput`+`mat-error`+`getFieldError()` + Angular 6 Foundation state components + Angular 7 Upload fichiers + Angular 8 Tests+smoke)** | **✓ (Angular 3→8 — HTTP, intercepteurs, server-state RxJS, Angular Material, Foundation states, FileCategory/AppFile/UploadService/UploadFormComponent, CDK harnesses, RouterTestingHarness)** | **✓ (224/224 ; Angular 3→7 + 16 nouveaux tests Angular 8 : RouterTestingHarness navigation/guards, MatFormFieldHarness/MatInputHarness login, MatSelectHarness/MatButtonHarness/MatProgressSpinnerHarness upload+loading)** | — | **TEST_SMOKE_READY** | `architect web-angular:build` SUCCESS · `architect web-angular:test` 224/224 ✅ · `npm audit` 0 vuln | Web Core Angular V1 Readiness Review |
