...
| AI Core | ✓ | ✓ (`CORE_SPECIFICATION.md`, `README.md` — AI Core 1, 2026-07-16) | — | — | ✓ (`prompt-registry.json` + validateur Node pur + redaction layer + context builder + provider fake + evaluation harness + décision Retrieval/RAG V1 — AI Core 2→7) | ✓ (tests Node + validation registry + redaction + context + provider + evaluation + décision RAG documentée) | — | **IMPLEMENTATION_PARTIELLE** | `validate-prompt-registry` + `node --test` + `quality-gates docs` | AI Core 8 — Governance/execution report schema |
