...
> `web-angular` : **`FORMS_MATERIAL_READY` → `FOUNDATION_STATES_READY`**. Prochaine action :
> Web Core Angular 7 — PermissionService + PermissionDirective ou Upload selon roadmap.
>
...
| Web Core Angular | ✓ | **✓ (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15)** | **ADR-035 (Validé, 2026-07-15)** | **✓ (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 `mat-form-field`+`matInput`+`mat-error`+`getFieldError()` + Angular 6 Foundation state components)** | **✓ (Angular 3→6 — HTTP, intercepteurs, server-state RxJS, Angular Material, Foundation states standalone signal inputs, a11y roles, tokens Enistere)** | **✓ (170/170 ; Angular 3→5 + 34 nouveaux tests Angular 6 : role=status/alert, signal inputs, outputs, actions optionnelles)** | — | **FOUNDATION_STATES_READY** | `architect web-angular:build` SUCCESS · `architect web-angular:test` 170/170 ✅ · `npm audit` 0 vuln | Web Core Angular 7 — PermissionService + PermissionDirective ou Upload |
