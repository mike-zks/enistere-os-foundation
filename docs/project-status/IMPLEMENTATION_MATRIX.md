...
| Mobile Core React Native | ✓ | ✓ | ✓ (004,008,010,011,012,015,016…) | **✓ (Expo SDK 55 + Expo Router)** | **✓ (RN 1 socle + RN 2 hardening : AuthEngine agnostique restore/signIn/signOut/refresh/clear, refresh coalescé, expiration ; SessionStore SecureStore + validation, access token mémoire ADR-015 ; API client `401`→refresh→retry ADR-011 ; gardes expired/refreshing ; seam `@enistere/api-client-fetch` ADR-016 ; TanStack Query ADR-012 ; ThemeProvider ADR-008/010 ; états standards)** | **✓ (21 tests `node --test` : auth-engine/session-store/api-client)** | — | **AUTH_SESSION_HARDENED** | typecheck + lint + **test 21/21** + **expo-doctor 19/19** verts (local) | **Mobile Core RN 3 — forms, validation & offline-ready primitives** (RHF + Zod ; puis intégration `api-client-fetch` réelle, Zustand/upload différés) |
...
| C3 | ADR-005/012/013/014/015 Validés | Code correspondant partiel | ADR-008 **partiel** (tokens + primitives UI Kit) ; ADR-009 **partiel** (web : Tailwind/Radix absents) ; **ADR-010 appliqué côté mobile** (ThemeProvider + composants maison, pas de NativeWind) ; **ADR-011/012 appliqués** (web + mobile : fetch + TanStack Query) ; **ADR-015 implémenté** (mobile secure storage : access token mémoire, refresh token SecureStore) ; ADR-005/013/014 décidés, partiels | Lecture « fait » erronée | Implémenter au fil des cores | IMPORTANTE |
...
| C6 | `cores/{cloud,mobile-react-native}` ont une spéc | Starter | `cloud` = PARTIEL/PAUSE ; **`mobile-react-native` `AUTH_SESSION_HARDENED`** (Expo SDK 55, socle + auth/session durci, 21 tests) ; ui-kit/web-nextjs `STARTER_INITIALISE` | Confusion spéc↔implémentation | Statut explicite (cette matrice) ; mobile = socle durci ≠ V1 complète | IMPORTANTE |
