...
>
> **Mise à jour AI Core 3 — Redaction layer pure + tests (2026-07-16)** :
...
>
> **Mise à jour AI Core 4 — Context builder allow-list (2026-07-16)** :
...
>
> **Mise à jour AI Core 6 — Evaluation harness initial (2026-07-16)** :
...
>
> **Mise à jour AI Core 7 — Retrieval/RAG design decision (2026-07-16)** :
> `docs/project-status/AI_CORE7_RETRIEVAL_RAG_DECISION.md` fixe le cadrage V1 :
> retrieval documentaire local par allow-list explicite, corpus Foundation versionné, Context Builder AI Core 4,
> redaction AI Core 3, Evaluation Harness AI Core 6 et revue humaine. Aucun embedding, vector DB, provider réel,
> SDK IA, clé API, appel réseau, index persistant, ingestion globale, workflow CI automatique, dépendance ou
> stockage de traces. Tout choix futur d'embedding/vector store/provider/service RAG persistant nécessitera une
> décision dédiée, potentiellement un ADR. `ai-core` reste **`IMPLEMENTATION_PARTIELLE`**.
>
> **Mise à jour AI Core 8 — Governance/execution report schema (2026-07-16)** :
> `src/reports/execution-report.mjs` + `src/reports/index.mjs` ajoutent le schema local
...
> rapport `AI_CORE_V1_READINESS_REVIEW.md`. §18 : **7/7 satisfaits**. §19 futur :
> **5/8 satisfaits + 3/8 partiels + 0 non satisfaits**. Décision :
> **`IMPLEMENTATION_PARTIELLE` → `IMPLEMENTATION_AVANCEE`**. `VALIDE_V1` est différé par B1 prompt runner
> gouverné, B2 retrieval source citation helper et B3 runbook d'usage AI Core.
>
> **Mise à jour AI Core 9 — Prompt execution runner (2026-07-16)** :
...
>
> **Mise à jour AI Core V1 Final Readiness Decision (2026-07-17)** :
> rapport `AI_CORE_V1_FINAL_READINESS_DECISION.md`. §19 : **8/8 satisfaits**. B1 prompt runner, B2
> retrieval citations et B3 usage runbook sont fermés. Décision :
...
>
> **Mise à jour Web Core Angular 6 — Composants Foundation Enistere (2026-07-16)** :
> `shared/components/loading-state/enistere-loading-state.component.{ts,html,scss,spec.ts}` : signal `message`/`size`, `MatProgressSpinnerModule`, `diameterFor` map, `role="status"`, `aria-live="polite"`, `[attr.aria-label]` dynamique ; 7 tests.
> `shared/components/empty-state/enistere-empty-state.component.{ts,html,scss,spec.ts}` : `input.required<string>()` `title`, `input<string>()` `description`/`actionLabel`, `output<void>()` `actionClicked`, `role="status"` ; 9 tests.
> `shared/components/error-state/enistere-error-state.component.{ts,html,scss,spec.ts}` : `input.required<string>()` `title`, `input<string>()` `description`/`retryLabel`, `output<void>()` `retried`, `role="alert"`, `aria-live="assertive"` ; 9 tests.
> `shared/components/success-state/enistere-success-state.component.{ts,html,scss,spec.ts}` : `input.required<string>()` `title`, `input<string>()` `description`/`actionLabel`, `output<void>()` `actionClicked`, `role="status"` ; 9 tests.
> SCSS : tokens Enistere `--enistere-color-status-danger/success`, `--enistere-color-foreground-default/muted`, `--enistere-spacing-4/8`, `--enistere-font-size-lg`, `--enistere-font-weight-semibold`.
> `home.component.html` badge `FOUNDATION_STATES_READY`. Tests : **170/170 ✅**.
> `web-angular` : **`FORMS_MATERIAL_READY` → `FOUNDATION_STATES_READY`**.
>
> **Mise à jour Web Core Angular 7 — Upload fichiers (2026-07-16)** :
> `core/upload/file-category.ts` (`FileCategory` 9 valeurs : IMAGE/DOCUMENT/AVATAR/VIDEO/AUDIO/IDENTITY_DOCUMENT/ATTACHMENT/MEDIA/OTHER + `FILE_CATEGORIES` label pairs).
> `core/upload/app-file.model.ts` (`AppFile {file, category, subjectId?}` + `MAX_FILE_SIZE_BYTES` 10 MB + `isValidAppFile` + `isAllowedFileType` whitelist 14 MIME + `describeFileForLog` → `{extension, sizeBytes}` jamais nom/path/contenu).
> `core/upload/uploaded-file-metadata.model.ts` (`UploadedFileMetadata {id, category}` — sans storageKey/bucket/signedUrl/ownerId).
> `core/upload/upload.service.ts` (`UploadService` injectable — `HttpClient.post()` + `FormData` (file, category, subjectId?) ; Content-Type intentionnellement absent ; `createRequestState()` pour le server-state ; erreurs 413/415/401 mappées via `errorInterceptor` + `AppApiError` existants).
> `features/upload/upload-form/upload-form.component.{ts,html,scss,spec.ts}` : Reactive Forms `{category: required, subjectId: maxLength(128)}` ; `signal<File|null>` + `signal<RequestState<UploadedFileMetadata>>` ; `errorMessage` computed (FileTooLarge/UnsupportedType/Unauthorized/RateLimited) ; états idle/loading/success/error via composants Foundation Angular 6 ; `reset()` + `ngOnDestroy`.
> `home.component.html` badge `UPLOAD_READY`. Tests : **205/205 ✅**.
> `web-angular` : **`FOUNDATION_STATES_READY` → `UPLOAD_READY`**.
>
> **Mise à jour Web Core Angular V1 Readiness Review (2026-07-16)** :
> Rapport `docs/project-status/WEB_ANGULAR_V1_READINESS_REVIEW.md`. 15 critères §29 vérifiés un par un.
> Après Angular 10 : **14 / 15 SATISFAIT** (§29.1 build, §29.2 router+guards, §29.3 auth flow + refresh seam, §29.4 tokens mémoire, §29.5 intercepteurs auth/refresh/log/error, §29.6 Reactive Forms, §29.7 Material 3 thème, §29.8 Foundation components, §29.10 tests auth, §29.11 tests composants 267/267, §29.12 HTTP typé, §29.13 PermissionService/PermissionDirective, §29.14 Chrome, §29.15 ng build production).
> **1 / 15 PARTIEL** (§29.9 CDK a11y) + **0 / 15 NON SATISFAIT**.
> Blockers V1 fermés : **B1 RefreshInterceptor/login API seam** (Angular 9) + **B2 PermissionService/PermissionDirective** (Angular 10). Réserve non-bloquante restante : R1 CDK a11y FocusTrap/LiveAnnouncer (ARIA HTML équivalent). R2 CI Angular gate fermée par Quality Web Angular CI gate (workflow `web-angular-ci.yml` + scope `quality-gates web-angular`).
> Cohérence ADR-035 vérifiée : Angular Material CDK + M3 ✅ · Reactive Forms ✅ · Foundation components ✅ · CDK harnesses ✅ · Signals ✅ · PrimeNG absent ✅.
> `home.component.html` badge `TEST_SMOKE_READY` → `VALIDE_V1`. Tests : **267/267 ✅**.
> `web-angular` : **`TEST_SMOKE_READY` → `VALIDE_V1`**.
>
> **Mise à jour Quality Web Angular CI gate (2026-07-16)** :
> `.github/workflows/web-angular-ci.yml` ajouté (job `web-angular` : `npm ci`, `npm run test:ci`, `npm run build`, `npm audit`) ; scope `web-angular` ajouté dans `quality-gates.mjs` ; `quality-report.mjs` suit désormais 9 scopes. R2 Angular fermée. Le check `web-angular` reste promotion recommandée, non appliquée automatiquement dans le ruleset `protect-main`.
>
> **Mise à jour Web Core Angular 8 — Tests + smoke (2026-07-16)** :
> `app.navigation.spec.ts` : 5 tests intégration router (`RouterTestingHarness`) — lazy-loaded routes, `authGuard` redirect `/dashboard`→`/login?returnUrl=...`, `guestGuard`, wildcard `/page-inconnue`→`/`.
> `features/auth/login/login.harness.spec.ts` : 5 tests CDK — `MatFormFieldHarness` (2 fields, labels "Adresse e-mail"/"Mot de passe"), `MatInputHarness.getType()` (email/password).
> `features/upload/upload-form/upload-form.harness.spec.ts` : 4 tests CDK — `MatSelectHarness` (9 options = FILE_CATEGORIES, labels), `MatButtonHarness.with({ text: 'Envoyer' })`, `MatProgressSpinnerHarness` en état loading.
> `shared/components/loading-state/enistere-loading-state.harness.spec.ts` : 2 tests CDK — `MatProgressSpinnerHarness` trouvé + `getMode() === 'indeterminate'`.
> Zéro nouvelle dépendance. `home.component.html` badge `UPLOAD_READY` → `TEST_SMOKE_READY`. Rapport `WEB_ANGULAR8_TESTS_SMOKE_REPORT.md`. Tests : **224/224 ✅**.
...
| `@enistere/api-contracts` | ✓ | n/a | ✓ (016) | ✓ | ✓ | ✓ (12 + `pack:dry-run` + tarball SHA-256 + consumer install) | ✓ (`PACKAGES_RELEASE_1_TARBALLS_REPORT.md`) | **VALIDE_V1** (GitHub Release tarball `0.1.0`) | build + generate:check ; `npm pack` OK ; asset `enistere-api-contracts-0.1.0.tgz` tag `packages-api-typescript-v0.1.0` | GitHub Packages npm registry (canal principal futur) |
| `@enistere/api-client-fetch` | ✓ | n/a | ✓ (011,012,016) | ✓ | ✓ | ✓ (30 + live 16/16 + `pack:dry-run` + tarball SHA-256 + consumer smoke) | ✓ (`PACKAGES_RELEASE_1_TARBALLS_REPORT.md`) | **VALIDE_V1** (GitHub Release tarball `0.1.0`) | live 16/16 ; **instancié (public + authentifié/BFF Auth + façade Files lecture) dans le Web Core** ; asset `enistere-api-client-fetch-0.1.0.tgz` tag `packages-api-typescript-v0.1.0` | GitHub Packages npm registry (canal principal futur) |
...
| Quality Core | ✓ | ✓ | — | — | ✓ (scripts `quality-gates` + `release-helper` + `quality-report` testés ; release process utilisé ; workflow L6 `web-angular-ci.yml` documenté) | ✓ (`QUALITY_CORE_V2_READINESS_REVIEW.md` + `QUALITY_CORE_ADVANCED_READINESS_REVIEW.md` + `QUALITY_CORE_RELEASE_HELPER_REPORT.md` + `QUALITY_CORE_COVERAGE_REPORTING_BASELINE.md` + `QUALITY_CORE_REQUIRED_CHECKS_ALIGNMENT.md` + `QUALITY_CORE_COVERAGE_STANDARDIZATION_DECISION.md` + `QUALITY_CORE_V1_READINESS_REVIEW.md` + `QUALITY_WEB_ANGULAR_CI_GATE_REPORT.md`) | ✓ | **VALIDE_V1** | `CORE_SPECIFICATION.md` + `QUALITY_GATES_MATRIX.md` + `BRANCH_PROTECTION_RUNBOOK.md` + `RELEASE_PROCESS_RUNBOOK.md` + `AI_PROMPT_GOVERNANCE.md` + 3 checklists + `scripts/quality-gates.mjs` (9 scopes) + `scripts/release-helper.mjs` + `scripts/quality-report.mjs` (9 scopes) + templates GitHub + prompts catalogués + release `foundation-v1.0.0` gouvernée + Docs Core connecté au gate docs + décision checks `images` + coverage UI Kit/Web/API reconnue + gate CI `web-angular` | — (V1 déclaré) |
| Web Core Angular | ✓ | **✓ (`CORE_SPECIFICATION.md` 32 §, `README.md` — Angular 1, 2026-07-15)** | **ADR-035 (Validé, 2026-07-15)** | **✓ (Angular 2 starter + Angular 3 auth/routing + Angular 4 HTTP/server-state + Angular 5 `mat-form-field`+`matInput`+`mat-error`+`getFieldError()` + Angular 6 Foundation state components + Angular 7 Upload fichiers + Angular 8 Tests+smoke + Angular 9 RefreshInterceptor/login API seam + Angular 10 PermissionService/PermissionDirective)** | **✓ (Angular 3→10 — HTTP, intercepteurs auth/refresh/log/error, server-state RxJS, Angular Material, Foundation states, FileCategory/AppFile/UploadService/UploadFormComponent, CDK harnesses, RouterTestingHarness, RBAC UX `*enisterePermission`)** | **✓ (267/267 ; Angular 9 ajoute `AuthApi` seam + `RefreshInterceptor` 401→refresh→retry unique ; Angular 10 ajoute `PermissionService` + `PermissionDirective`)** | **✓ (`WEB_ANGULAR_V1_READINESS_REVIEW.md` mis à jour — 14/15 §29 SATISFAIT + 1/15 PARTIEL + 0/15 NON SATISFAIT ; zéro blocker V1 ; R2 CI fermée ; rapports `WEB_ANGULAR9_REFRESH_INTERCEPTOR_REPORT.md` + `WEB_ANGULAR10_PERMISSION_RBAC_REPORT.md` + `QUALITY_WEB_ANGULAR_CI_GATE_REPORT.md`)** | **VALIDE_V1** | `architect web-angular:build` SUCCESS · `architect web-angular:test` 267/267 ✅ · `npm audit` 0 vuln · `web-angular-ci.yml` L6 + `quality-gates web-angular` | — (V1 déclaré) |
...
| `tools/` vide ; `examples/` a un premier smoke consommateur (`examples/api-client-node`) ; quickstart développeur livré | MINEURE |
