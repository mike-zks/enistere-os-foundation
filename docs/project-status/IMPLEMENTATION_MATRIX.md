...
| Web Core Next.js | ✓ | ✓ | ✓ (004,005,006,009,011,012,016…) | **✓** | **✓ (App Router + UI Kit + API publique Health + TanStack Query + BFF Auth + me/authorization + session state + layout protégé serveur read-only Option C + hydratation + page /protected + page de connexion /login : formulaire, login BFF, returnTo interne assaini, navigation replace/refresh)** | **✓ (263, a11y + sonde HTTP + preuve API réelle Auth/session + protégé 26/26 + login 22/22)** | **✓ (gouvernance 2026-06-10 `WEB_CORE_GOVERNANCE_REVIEW.md` + revue Auth V1 `WEB_AUTH_V1_REVIEW.md` → `AUTH_WEB_V1_STABLE_WITH_RESERVATIONS`)** | **IMPLEMENTATION_PARTIELLE** | build/lint/typecheck/**263 tests ×2** verts + couverture + preuves API réelles (Auth/session ; protégé ; login ; **runtime V1 33/33** : nominal+erreurs+refresh+droits sans nouveau JWT ; **aucun open redirect**, **aucune fuite token/mot de passe**, bundle sans secret) | **Web Core — états UI & composants structurels** (`CORE_SPEC` §3/§4) ; CI + E2E navigateur recommandés (réserves V1) |
