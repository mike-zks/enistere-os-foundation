...
> Vérifiée depuis le repository (2026-06-10). Légende des statuts officiels : `ABSENT`,
...
| Cloud Core | ✓ | ✓ | ✓ (013,014,007…) | — | **cadrage (CC1) + CI runtime API (CC2) + CI E2E navigateur (CC3)** : `api-runtime-ci.yml` (PG+MinIO, migrations, unit+e2e, openapi:check) **+ `web-e2e-ci.yml`** (stack réelle + Playwright/Chromium : Health/Auth/Files) | **e2e API + E2E navigateur en CI** (niveaux 2–3) | — | **IMPLEMENTATION_PARTIELLE** | **quatre workflows CI** niveaux 1–4 partiel (`ci`/`api-runtime`/`web-e2e`/**`registry`** — Registry CI verte sur `main`, **images GHCR publiques**) **+ gouvernance CC4 + staging manuel CC6** (`cores/cloud/staging/` + runbooks, `CADRE_MANUEL_DOCUMENTE`) ; **aucune exécution de déploiement** | **Cloud Core 7 — exécution réelle staging** (serveur + secrets hors dépôt) ou dry-run / durcissement registry |
...
| Conteneurisation (Docker) | ADR-014 | **Dockerfiles API/Web** (multi-stage, non-root ; Web standalone) + `.dockerignore` ; **compose staging exemple** (CC6) | ✓ (`docker build` + `compose config` OK) | **PARTIELLEMENT_IMPLEMENTE** | compose de prod, Traefik |
| Déploiement staging | ADR-013 | **runbooks + compose/`.env` exemples** (CC6, `cores/cloud/staging/`) | — (manuel, non exécuté) | **CADRE_MANUEL_DOCUMENTE** | exécution réelle sur serveur (Cloud Core 7) |
