...
| ADR-015 | Stockage mobile sécurisé | Validé | **PARTIELLEMENT_IMPLEMENTE** | mobile/api | **Mobile Core (RN 1→3)** : `SecureStorage` + `ExpoSecureStorage` (SecureStore) ; **access token en mémoire**, **refresh token en SecureStore** via `SessionStore` (validation du format, fail-soft → `null`) ; **purge complète au logout** (storage + cache TanStack Query) ; tokens **hors logs** ; offline (RN 3) = **queue mémoire non persistée, sans donnée sensible** (§19). Reste : biométrie/Keychain/MMKV **selon projet**, persistance offline (ADR-029) |
