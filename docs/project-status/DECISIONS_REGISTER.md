...
> de chaque décision validée. Vérifié depuis le repository (2026-07-08).
>
...
| ADR-008 | Design tokens UI Kit | Validé | **PARTIELLEMENT_IMPLEMENTE** | ui-kit/web/mobile | `@enistere/ui-kit` : tokens + **12 primitives Web** (Button/Input/Label/Text/Spinner/VisuallyHidden + Alert/Card/FormField + **Dialog/Select/Toast**, UI Kit 4 — CSS pilotée par tokens, aucun hex ; **121 tests**) ; bibliothèque complète à venir |
...
- **ADR-008/009/010** — UI Kit : **partiellement fait** (tokens + **12 primitives Web**, React 19 — Web UI 1
  ajoute Alert/Card/FormField, UI Kit 4 ajoute Dialog/Select/Toast) ; **consommé par le Web Core** (états UI standardisés, compatibilité Next RSC vérifiée avec `Dialog` marqué `'use client'`). **ADR-010 mobile FAIT** :
