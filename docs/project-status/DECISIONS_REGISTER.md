...
| ADR-035 | Angular UI : Angular Material (CDK + M3) contrôlé par tokens Enistere + composants maison | Validé | **PARTIELLEMENT_IMPLEMENTE** | ui-kit, web-angular | Option D : Angular Material (CDK + Material 3) contrôlé par tokens Enistere (ADR-008) via `mat.define-theme()` / CSS custom properties `--mat-*` + composants maison Enistere Angular ciblés + Reactive Forms obligatoire + `@angular/cdk/testing` + Angular Signals / NgRx différé projet dérivé. Pas de PrimeNG, pas de shadcn/Radix côté Angular. Adaptateur OpenAPI Angular (ADR-016 §F) décidé par preuve dans Web Core Angular. Décision 2026-07-15. **Angular 1 (2026-07-15)** : `cores/web-angular/CORE_SPECIFICATION.md` (32 §) + `README.md` livrés — `web-angular` passe à `SPECIFICATION_DOCUMENTAIRE`. **Angular 2 (2026-07-15)** : starter minimal créé — Angular 22.0.6, Material/CDK 22.0.4, thème `mat.define-theme()` + tokens Enistere `--enistere-*` → `--mat-sys-*`, dark mode `[data-theme='dark']`, 8/8 tests ✅ (ChromeHeadless), build SUCCESS (340 KB) — `web-angular` passe à `STARTER_INITIALISE`. **Angular 3 (2026-07-16)** : auth/routing shell livré — `AuthService` Signals read-only, token mémoire privé, guards, `AuthInterceptor`, routes `/login`/`/dashboard` — `web-angular` passe à `AUTH_ROUTING_READY`. **Angular 4 (2026-07-16)** : client HTTP + server-state RxJS livrés — `APP_BASE_URL`, erreurs typées, intercepteurs `auth→log→error`, logs sûrs, `RequestState<T>` ; `web-angular` passe à `HTTP_SERVER_STATE_READY`. **Angular 5 (2026-07-16)** : Reactive Forms + Angular Material livrés — `mat-form-field` outline, `matInput`, `mat-error`, `getFieldError()` pur (required/email/minlength/maxlength/pattern), `mat-flat-button`, SCSS nettoyé ; 136/136 tests ✅ ; `web-angular` passe à `FORMS_MATERIAL_READY`. **Angular 6 (2026-07-16)** : 4 composants Foundation standalone — `EnistereLoadingStateComponent` (`MatProgressSpinnerModule`, signal `message`/`size`, `role="status"`, `aria-live="polite"`, `aria-label` dynamique), `EnistereEmptyStateComponent` (`input.required<string>()` `title`, action optionnelle, `role="status"`), `EnistereErrorStateComponent` (`role="alert"`, `aria-live="assertive"`, retry optionnel), `EnistereSuccessStateComponent` (`role="status"`, action optionnelle) ; tokens Enistere `--enistere-color-status-danger/success/foreground-*`, `--enistere-spacing-*` ; 170/170 tests ✅ ; `web-angular` passe à `FOUNDATION_STATES_READY`. **Angular 7 (2026-07-16)** : primitives upload génériques — `FileCategory` (9 valeurs API screaming-case) + `FILE_CATEGORIES` labels ; `AppFile {file, category, subjectId?}` + `MAX_FILE_SIZE_BYTES` 10 MB + `isValidAppFile` + `isAllowedFileType` whitelist 14 MIME + `describeFileForLog({extension, sizeBytes})` (jamais nom/path/contenu/token) ; `UploadedFileMetadata {id, category}` sans champ interne ; `UploadService` `HttpClient.post()` + `FormData` Content-Type intentionnellement absent (boundary posé par le navigateur), `createRequestState`, mapping 413/415/401 via `errorInterceptor`+`AppApiError` ; `UploadFormComponent` Reactive Forms (`category` required, `subjectId` maxLength 128), `signal<File|null>`, `signal<RequestState<...>>(idleState())`, `errorMessage` computed, états idle/loading/success/error via composants Foundation Angular 6 ; 205/205 tests ✅ ; `web-angular` passe à `UPLOAD_READY`. **Angular 8 (2026-07-16)** : tests + smoke livrés — `RouterTestingHarness` (navigation lazy-loaded + guards + redirects) + CDK harnesses Material (`MatFormFieldHarness`/`MatInputHarness` login, `MatSelectHarness`/`MatButtonHarness`/`MatProgressSpinnerHarness` upload, spinner LoadingState) ; 224/224 tests ✅ ; `web-angular` passe à `TEST_SMOKE_READY`. Prochaine : Web Core Angular V1 Readiness Review. |
...
> + Angular 7 + Angular 8. `web-angular` est `TEST_SMOKE_READY`; la revue V1
