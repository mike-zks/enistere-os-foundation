...
| ADR-010 | Stack UI React Native | Validé | **PARTIELLEMENT_IMPLEMENTE** *(statut inchangé en RN 14)* | ui-kit/mobile | **ThemeProvider mobile + composants maison LIVRÉS** (Mobile Core RN 1→3 : `ThemeProvider` light/dark, `Screen`/`Text`/`Button` + primitives form `FormField`/`FormLabel`/`FormError`/`TextInputField`, token-driven, états pressed/disabled/loading/invalid, cible tactile a11y, erreurs en live region — **pas de NativeWind ni lib UI**) ; bridge tokens placeholder en attendant la surface mobile `@enistere/ui-kit`. **+ Mobile RN 14 — primitives a11y génériques** (§16) : `buildA11yProps`/`A11yState` (états **disabled/focused/pressed/invalid** + RN `accessibilityState`) + annonce lecteur d'écran (`createA11yService`/`A11yAdapter` + placeholder, **logs sûrs** `{length,assertive}` — **aucun contenu/label loggé**) ; **aucun `AccessibilityInfo` réel/provider global/stockage/dépendance** ; props à appliquer par les composants (projets dérivés / UI Kit). **Statut ADR inchangé** (couverture a11y élargie, pas de complétion) |
...
- **ADR-008/009/010** — UI Kit : **partiellement fait** (tokens + **9 primitives Web**, React 19 — Web UI 1
  ajoute Alert/Card/FormField) ; **consommé par le Web Core** (états UI standardisés). **ADR-010 mobile FAIT** :
  ThemeProvider + composants maison (Screen/Text/Button + primitives form, token-driven, a11y) — **pas de
  NativeWind**. **ADR-009 reste partiel : Tailwind/Radix/shadcn volontairement absents** (non requis). Restent :
  composants supplémentaires (Web) et **surface tokens mobile** `@enistere/ui-kit` (bridge placeholder en attendant).
