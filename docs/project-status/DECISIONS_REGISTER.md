...
| ADR-034 | Flutter UI : Material 3 vs composants maison | Validé | **DECIDE_NON_IMPLEMENTE** | ui-kit/mobile-flutter | Décision V3 : futur Mobile Core Flutter = **Material 3 contrôlé par tokens Enistere + composants maison ciblés**. Material 3 est le moteur Flutter, pas l'identité visuelle autonome. Aucun starter Flutter, aucune dépendance et aucune spec encore livrés. Prochaine étape : Mobile Core Flutter 1 — Core specification. |
...
`ADR-017→033` et `ADR-035→038` sont listés dans [`../../docs/adr/ADR_BACKLOG.md`](../../docs/adr/ADR_BACKLOG.md)
avec le statut « À rédiger » (queues, monitoring, crash reporting, E2E, i18n, icônes, maps, offline,
backups, déploiement avancé, charts, Angular UI, observabilité distribuée, feature flags,
analytics…). **Aucun fichier ADR correspondant n'existe** → `NON_APPLICABLE_ACTUELLEMENT` jusqu'à
rédaction. Bloquant futur notable : **ADR-035** (Angular UI) avant `web-angular`.
