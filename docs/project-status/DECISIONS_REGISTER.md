...
| ADR-016 | OpenAPI + clients typés | Validé | **IMPLEMENTE** | api/web/mobile | contrat + packages ; **consommés par le Web Core** (types `SchemaOf<>` Health/Auth/Files ; client instancié BFF) **ET par le Mobile Core (RN 4)** : `@enistere/api-client-fetch` + `@enistere/api-contracts` **réellement intégrés** (liés `file:` + Metro, core autonome ; `createEnistereApiClient` + `MobileAuthSessionAdapter` + `EnistereAuthApi` sur `/auth/login`/`/auth/refresh` typés ; bundle Metro prouvé) — **aucun DTO recopié, aucune interface cliente manuelle** (§39). **Distribution décidée** (2026-07-17) : GitHub Packages npm registry, repli GitHub Release tarballs. **Préparation publish-ready livrée** : `private:false`, `publishConfig`, relation SemVer `0.1.0`, `npm pack --dry-run` OK. **Packages Release 1** (2026-07-18) : distribution effective par repli GitHub Release tarballs, tag `packages-api-typescript-v0.1.0`, assets `enistere-api-contracts-0.1.0.tgz` + `enistere-api-client-fetch-0.1.0.tgz`, installation consommateur externe et smoke import vérifiés. GitHub Packages npm registry reste le canal principal futur. **Web Angular 4** pose seulement la couche `HttpClient`/erreurs/config ; aucun adaptateur OpenAPI Angular n'est encore livré. Reste : adaptateurs Angular/Dart futurs + éventuel registry npm GitHub Packages |
...
- **ADR-016 (reste)** — **publication** des packages (toujours non publiés) ; **strategie de
  distribution décidée** (`PACKAGES_DISTRIBUTION_DECISION.md` : GitHub Packages, repli GitHub Release
  tarballs) ; **publish-ready livré** (`PACKAGES_DISTRIBUTION_2_PUBLISH_READY_REPORT.md` :
  `private:false`, `publishConfig`, SemVer `0.1.0`, `npm pack --dry-run`) ; **intégration cores FAITE**
...
> + Angular 7 + Angular 8 + Angular 9. `web-angular` est `IMPLEMENTATION_AVANCEE`; la revue V1
> finale reste différée par B2 PermissionService/PermissionDirective.
