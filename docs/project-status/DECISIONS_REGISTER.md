...
| ADR-041 | Build system Spring Boot = Maven | Validé | **DECIDE_NON_IMPLEMENTE** | api-spring | Maven comme build system V1 : `pom.xml`, Spring Boot Parent POM, `mvn verify`, Maven Wrapper. Gradle autorisé uniquement par exception documentée dans un projet dérivé. Implémentation attendue en Spring Boot 2 (starter) : `pom.xml` + `mvnw`. |
