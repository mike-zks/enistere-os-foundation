...
- **ADR-016 (reste)** — **publication** des packages (toujours non publiés/privés) ; **strategie de
  distribution décidée** (`PACKAGES_DISTRIBUTION_DECISION.md` : GitHub Packages, repli GitHub Release
  tarballs) ; **intégration cores FAITE** (Web + **Mobile RN 4** : `@enistere/api-client-fetch`
  consommé) ; adaptateurs Angular/Dart futurs.
