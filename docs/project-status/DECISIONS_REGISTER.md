...
| ADR-035 | Angular UI : Angular Material (CDK + M3) contrôlé par tokens Enistere + composants maison | Validé | **PARTIELLEMENT_IMPLEMENTE** | ui-kit, web-angular | Option D : Angular Material (CDK + Material 3) contrôlé par tokens Enistere (ADR-008) via `mat.define-theme()` / CSS custom properties `--mat-*` + composants maison Enistere Angular ciblés + Reactive Forms obligatoire + `@angular/cdk/testing` + Angular Signals / NgRx différé projet dérivé. Pas de PrimeNG, pas de shadcn/Radix côté Angular. Adaptateur OpenAPI Angular (ADR-016 §F) décidé par preuve dans Web Core Angular. Décision 2026-07-15. **Angular 1→9 (2026-07-15/16)** : spécification, starter Angular 22, auth/routing shell, HttpClient/server-state RxJS, Reactive Forms + Material, Foundation state components, upload fichiers, tests/smoke CDK et RefreshInterceptor/login API seam livrés. **Readiness mise à jour après Angular 9 (2026-07-16)** : `web-angular` reste `IMPLEMENTATION_AVANCEE`, `VALIDE_V1` différé par B2 PermissionService/PermissionDirective. Score : 13/15 §29 SATISFAIT + 1/15 PARTIEL + 1/15 NON SATISFAIT ; 248/248 tests ✅. Prochaine : Angular 10 — PermissionService + PermissionDirective. |
