...
| ADR-035 | Angular UI : Angular Material (CDK + M3) contrôlé par tokens Enistere + composants maison | Validé | **PARTIELLEMENT_IMPLEMENTE** | ui-kit, web-angular | Option D : Angular Material (CDK + Material 3) contrôlé par tokens Enistere (ADR-008) via `mat.define-theme()` / CSS custom properties `--mat-*` + composants maison Enistere Angular ciblés + Reactive Forms obligatoire + `@angular/cdk/testing` + Angular Signals / NgRx différé projet dérivé. Pas de PrimeNG, pas de shadcn/Radix côté Angular. Adaptateur OpenAPI Angular (ADR-016 §F) décidé par preuve dans Web Core Angular. Décision 2026-07-15. **Angular 1→8 (2026-07-15/16)** : spécification, starter Angular 22, auth/routing shell, HttpClient/server-state RxJS, Reactive Forms + Material, Foundation state components, upload fichiers et tests/smoke CDK livrés. **Angular V1 Readiness Review (2026-07-16)** : `web-angular` passe à `IMPLEMENTATION_AVANCEE`, mais `VALIDE_V1` est différé par B1 RefreshInterceptor/login API seam + B2 PermissionService/PermissionDirective. Score : 11/15 §29 SATISFAIT + 3/15 PARTIEL + 1/15 NON SATISFAIT ; 224/224 tests ✅. Prochaine : Angular 9 — RefreshInterceptor + login API seam. |
