...
| §29.3 | Le flow auth est opérationnel (login / logout / refresh / session restore) | ✅ SATISFAIT | Angular 9 : `AuthApi` seam injectable + `PlaceholderAuthApi`, `AuthService.login()` Observable, `logout()` best-effort + purge mémoire, `restoreSession()` cold-start, `refreshSession()` coalescé + purge sur échec. Access token en mémoire uniquement. | — |
...
| §29.5 | Les intercepteurs HttpClient fonctionnent (auth, refresh, erreurs) | ✅ SATISFAIT | `authInterceptor` ✅ (Bearer injecté, auth-endpoints exclus) · `refreshInterceptor` ✅ (401→refresh coalescé→retry unique→logout/purge si échec, aucune boucle, FormData sans Content-Type forcé) · `errorInterceptor` ✅ (AppApiError) · `logInterceptor` ✅ (path sanitized). | — |
...
### B1 — RefreshInterceptor / login API seam (§29.3, §29.5) — FERMÉ par Angular 9

**Nature** : ancien blocker fonctionnel V1, fermé le 2026-07-16.

- `AuthService.login()` passe par un `AuthApi` injectable et retourne un `Observable<void>`.
- `RefreshInterceptor` livre le flux 401 → `refreshSession()` coalescé → retry unique → logout/purge si échec.
- Les endpoints `/auth/login`, `/auth/refresh`, `/auth/logout` sont exclus de l'injection Bearer et du refresh automatique.
- Le retry ne force pas `Content-Type` sur `FormData`.
- Access token : mémoire uniquement, jamais dans le signal public, jamais dans les logs, jamais dans `localStorage`.
